...
// ─────────────────────────────────────────────────────────────
// 🔐 Multi-Tenant User-Scoped Query Helper
// Ensures every DB query is filtered to the current user's telegram_id
// window.CURRENT_USER_ID is set by verifyAuth() in index.html
// ─────────────────────────────────────────────────────────────
function getUID() {
  return window.CURRENT_USER_ID || 1191760477;
}
function userQuery(tableName) {
  return db.from(tableName).select('*').eq('telegram_id', getUID());
}
...
async function renderHomeOverview(period = activeHomePeriod) {
  try {
    ...
    // Update greeting with the actual logged-in user's name
    const greetEl = document.getElementById('homeGreetName');
    if (greetEl && window.IS_TELEGRAM_USER && window.CURRENT_USER_NAME) {
      greetEl.textContent = `د. ${window.CURRENT_USER_NAME}`;
    }
