...
export function getLiveLifeOsPrompt(studentName = 'د. عبدالله', academicInfo = {}) {
  ...
  if (prefs.islamic !== false) {
    dynamicSections += `
- 📖 القرآن والصلاة (${studentName}): استخرج السورة وعدد الصفحات، والصلوات (fajr, dhuhr, asr, maghrib, isha) بالقيمة "حاضر في المسجد 🟢"، والسنن الرواتب (2, 4, 6) كرقم في prayer_habits.sunan_rawatib، والصيام وصلاة الضحى والوتر في fasting_worship. ⚠️ لا تسجل صلاة لم يدخل أذانها بعد (الوقت الحالي: ${cairoTimeStr}).`;
  }
  if (prefs.gym === true) {
    dynamicSections += `
- 🏋️‍♂️ الجيم: استخرج وقت التمرين بالدقائق، والعضلات المستهدفة وجرامات البروتين والماء في data.fitness_gym.`;
  }
...
1. 🌟 أنواع النوايا المدعومة (detected_type):
   - "life_actions": تسجيل أنشطة يومية (مذاكرة، صلاة، مصاريف، مهام، مواعيد، قرآن...).
...
{
  "detected_type": "life_actions", // أو "schedule_management" أو "modules_management" أو "recurring_reminder" أو "what_to_do_now" أو "bot_guidance" أو "conversational_chat"
  "conversational_reply": null, // نص الرد الحواري المباشر بالعامية المصرية إذا كانت النية حوارية أو إرشادية أو تعديل جدول
  "feature_tip": "💡 تريكة: تقدر تقولي بصوتك 'فكرني بعد صلاة العصر بساعة أراجع أطفال' وهفكرك تلقائياً!",
  "schedule_action": null, // "query" | "add" | "replace" | "prompt_details"
  "schedule_items": [], // [{ "course_code": "CAD402", "title": "سيكشن كارديو", "day_of_week": "الأحد", "start_time": "09:00", "location": "المستشفى" }]
  "recurring_reminder": null, // { "title": "سورة الكهف والصلاة على النبي", "frequency": "weekly", "day_of_week": "الجمعة", "time_24h": "10:00" }
