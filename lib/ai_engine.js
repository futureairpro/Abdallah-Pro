...
// Ultra-Fast Supported Gemini Flash Models Hierarchy (Ranked strictly by lowest latency ~400-700ms)
export const GEMINI_MODELS = [
  'gemini-flash-lite-latest', // ~450-550ms (Primary Ultra-Fast)
  'gemini-3.5-flash-lite',    // ~500-600ms (High-Speed Backup)
  'gemini-3-flash-preview',    // ~1400ms (Medium-Speed Fallback)
  'gemini-3.5-flash'          // ~1800ms (Rock-Solid Fallback)
];
...
8. 🕌 الصلاة والسنن والنوم والعبادات (prayer_habits & fasting_worship):
   ...
   - إذا ذكر أنه صلى سنة مؤكدة أو من السنن الرواتب الـ 12 ركعة المؤكدة عن النبي ﷺ:
     * سنة الفجر = 2 ركعة
     * سنة الظهر القبلية = 4 ركعات (أو 2 إذا حدد ركعتين)
     * سنة الظهر البعدية = 2 ركعة
     * سنة الظهر كاملة (قبلية وبعدية) = 6 ركعات
     * سنة المغرب = 2 ركعة
     * سنة العشاء = 2 ركعة
     * ضع عدد الركعات دائماً كرقم Number في prayer_habits.sunan_rawatib (مثال: 2 لسنة الفجر، 4 لسنة الظهر القبلية، 2 لسنة المغرب، 2 لسنة العشاء).
   - إذا ذكر صيام (مثل "صايم", "صمت سنة الإثنين", "صيام تطوع", "صيام قضاء", "صيام رمضان") أو صلاة الضحى أو صلاة الوتر:
     * ضعها في fasting_worship (مثل fasting_type: "صيام سنة", fasting_completed: true, duha_prayer_done: true, witr_prayer_done: true).
   - ⚠️ قاعدة التحقق الزمني الصارمة من الصلوات (Time-Gate Rule):
     * مواقيت الصلاة لليوم بالقاهرة: الفجر (${prayers.times.fajr}) | الظهر (${prayers.times.dhuhr}) | العصر (${prayers.times.asr}) | المغرب (${prayers.times.maghrib}) | العشاء (${prayers.times.isha}) والوقت الحالي هو ${cairoTimeStr}.
     * ممنوع منعاً باتاً تسجيل أي صلاة كـ "حاضر في المسجد 🟢" إذا كان موعد أذانها لم يدخل بعد في توقيت اليوم الحالي! (مثال: إذا كان الوقت قبل أذان العشاء، لا تسجل العشاء أبداً واجعل isha: null دائماً حتى لو ذكر المستخدم صلوات اليوم كلها).
   - وإذا ذكر أذكار الصباح أو المساء ضعها في adhkar_morning / adhkar_evening كـ true في prayer_habits.
