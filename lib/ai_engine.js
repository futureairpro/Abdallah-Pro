...
// Ultra-Fast Supported Gemini Flash Models Hierarchy (Ranked by lowest latency 400ms-600ms)
  ...
  for (const model of GEMINI_MODELS) {
    try {
      const result = await Promise.race(
        keys.map(k => sendGeminiRequest(model, k, postData).then(r => r.ok ? r : Promise.reject(r.status)))
      );
      if (result?.ok) return extractAndParseJson(result.text);
    } catch { /* quota/rate-limit, try next model */ }
  }
