...
// 🔄 Smart Round-Robin Key Counter (module-level, shared across all requests)
// Each incoming request picks a different starting key → quota distributed evenly across users
let _keyRoundRobinIdx = 0;
function getRotatedKeys(keys) {
  if (keys.length === 0) return keys;
  const startIdx = _keyRoundRobinIdx % keys.length;
  _keyRoundRobinIdx = (_keyRoundRobinIdx + 1) % keys.length;
  // Rotate array so the picked key is first, keeping fallback order intact
...
export async function parseWithGeminiPool(input, keyInput, isAudio = false, studentName = 'د. عبدالله') {
  ...
  // 🔄 Rotate starting key per request so quota is spread evenly across all users
  ...
  for (const model of GEMINI_MODELS) {
    for (const key of rotatedKeys) {
      ...
      // If quota exceeded on this key, try next key immediately
      if (!res.isQuota && !res.isForbidden) break; // Non-quota error → skip remaining keys for this model
    }
  }

  throw new Error('تعذر معالجة الطلب مؤقتاً.');
}
...
export async function talkWithEnglishCoach(input, keyInput, isAudio = false) {
  ...
  for (const key of rotatedKeys) {
    for (const model of GEMINI_MODELS) {
      const res = await sendGeminiRequest(model, key, postData);
      if (res.ok) return extractAndParseJson(res.text);
      if (!res.isQuota && !res.isForbidden) break;
    }
  }
