...
// ==============================================================================
export function getLiveLifeOsPrompt() {
  const now = new Date();
  ...
  return `أنت العقل المدبر والمدير التنفيذي والمستشار الشخصي لمنظومة "رحلة عبدالله" (Abdullah's Journey OS).
...
1. ⏰ المواعيد والتذكيرات (appointments & reminders):
   - إذا طلب د. عبدالله تذكيراً بوقت محدد أو بعد مدة زمنية (مثال: "ذكرني بعد ساعة بتصوير المحل", "فكرني كمان 30 دقيقة أكلم فلان", "عندي ميعاد الساعة 8 مساءً", "ذكرني بكرة الساعة 9 الصبح"):
...
9. 📝 صياغة summary_text:
   - يجب أن يذكر summary_text تفاصيل ما تم فهمه وتوثيقه بوضوح تام مع ذكر الأرقام والمواعيد والمدد بدقة بالغة.
...
export async function parseWithGeminiPool(input, keyInput, isAudio = false) {
  ...
  const systemPrompt = getLiveLifeOsPrompt();
  const parts = [];
  if (isAudio) {
  ...
  await sleep(1500);
  for (const currentKey of keys) {
    const retryRes = await sendGeminiRequest('gemini-3.6-flash', currentKey, postData);
