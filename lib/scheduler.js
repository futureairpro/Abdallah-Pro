...
const inMemorySentTracker = new Map();
let memoryCache = {};
let lastCacheFetchTime = 0;
const CACHE_TTL_MS = 60000; // 60 seconds local cache within execution cycle

// 🔄 Persistent Circular Queue Rotation Engine for Curricula (Guarantees zero repeat until full cycle)
export async function getRotatingCurriculumItem(category, items, idKey = 'id') {
  if (!items || items.length === 0) return null;
  try {
    const tracker = await getTrackerFromDb();
    if (!tracker._seen_capsules || typeof tracker._seen_capsules !== 'object') {
      tracker._seen_capsules = {};
    }
    const seenList = Array.isArray(tracker._seen_capsules[category]) ? tracker._seen_capsules[category] : [];
    const seenSet = new Set(seenList);

    let available = items.filter(it => !seenSet.has(it[idKey] || it.id));
    if (available.length === 0) {
      // Completed full curriculum cycle! Reset and restart
      tracker._seen_capsules[category] = [];
      available = items;
    }

    const chosen = available[0];
    const chosenId = chosen[idKey] || chosen.id;
    tracker._seen_capsules[category] = [...(tracker._seen_capsules[category] || []), chosenId];

    // Mark in persistent tracker
    await markSent(`capsule_cycle_${category}_${chosenId}`, 86400000 * 30);
    return chosen;
  } catch (e) {
    console.warn('getRotatingCurriculumItem warn:', e.message);
    return items[0];
  }
}
...
async function getTrackerFromDb() {
  const now = Date.now();
  if (memoryCache && Object.keys(memoryCache).length > 0 && (now - lastCacheFetchTime < CACHE_TTL_MS)) {
    return memoryCache;
  }
  try {
    ...
    if (error) {
      console.warn('[NotificationTracker DB Fetch Error]:', error.message);
      return memoryCache || {};
    }

    if (row?.data && typeof row.data === 'object') {
      memoryCache = { ...(memoryCache || {}), ...row.data };
    }
...
export async function hasSent(key, ttlMs = 86400000) {
  try {
    const now = Date.now();
    // 1. Check in-process memory tracker first (immune to DB outages/quota issues)
    if (inMemorySentTracker.has(key)) {
      const ts = inMemorySentTracker.get(key);
    ...
    return (now - sentTimestamp) < ttlMs;
  } catch (e) {
    return false;
  }
}
...
export async function markSent(key, ttlMs = 86400000) {
  try {
    const now = Date.now();
    // 1. Immediately store in RAM tracker
    inMemorySentTracker.set(key, now);

    // Prune in-memory tracker
    if (inMemorySentTracker.size > 2000) {
      for (const [k, v] of inMemorySentTracker.entries()) {
        if (now - v > 86400000 * 2) inMemorySentTracker.delete(k);
      }
    }
    ...
    if (error) {
      console.warn('[NotificationTracker DB Save Error]:', error.message);
    }
  } catch (e) {
...
async function runUserCycle(bot, user) {
  ...
  if (prefs.english !== false) {
    ...
    // ============================================================================
  // 2. 🩺 كويزات الموديولات الطبية الأكاديمية بالتكرار المتباعد (Medical Spaced Quizzes)
  // ============================================================================
  if (prefs.academic !== false) {
    try {
      const nonMedicalCodes = ['SHARIA', 'SAHABA_SPOTLIGHT', 'QURAN_ASBAB', 'BUKHARI', 'DISCIPLINE', 'PROPHETIC_LEADERSHIP'];
      const { data: dueQuizzes } = await supabase
        .from('medical_spaced_quizzes')
        .select('*')
        .ilike('topic', `[UID:${chatId}]%`)
        .not('course_code', 'in', `(${nonMedicalCodes.map(c => `"${c}"`).join(',')})`)
        .lte('next_review_at', new Date().toISOString())
        .eq('is_mastered', false)
      ...
      if (dueQuizzes && dueQuizzes.length > 0) {
        ...
        if (!alreadySent) {
          ...
          let options = null;
          let correctIdx = 0;
          let explanation = quiz.explanation || quiz.answer_and_explanation || 'إجابة من مذكرة الموديول';
          ...
          if (quiz.doctor_pearl && quiz.doctor_pearl.includes('<<<QUIZ_META_START>>>')) {
            try {
              ...
              if (parsedMeta.options && Array.isArray(parsedMeta.options) && parsedMeta.options.length >= 2) {
                options = parsedMeta.options;
                correctIdx = Number(parsedMeta.correct_index || 0);
                if (parsedMeta.explanation) explanation = parsedMeta.explanation;
              }
            } catch (e) {}
          } else if (quiz.options && Array.isArray(quiz.options) && quiz.options.length >= 2) {
            options = quiz.options;
            correctIdx = Number(quiz.correct_option_index || 0);
          }

          const cleanTopic = (quiz.topic || '').replace(/\[UID:\d+\]\s*/g, '').trim();

          if (options && options.length >= 2) {
            const questionText = `[${quiz.course_code || 'MED'}] ${quiz.question}`.substring(0, 300);
            const formattedOptions = options.map(o => String(o).substring(0, 100));
            ...
            const pollMessage = await bot.telegram.sendPoll(chatId, questionText, formattedOptions, {
              type: 'quiz',
              correct_option_id: Math.max(0, Math.min(formattedOptions.length - 1, correctIdx)),
            ...
            if (pollMessage && pollMessage.poll) {
              ...
              await supabase.from('medical_spaced_quizzes').update({
                repetition_level: Math.max(1, (quiz.repetition_level || 0) + 1),
                doctor_pearl: `<<<QUIZ_META_START>>>${JSON.stringify(metaObj)}<<<QUIZ_META_END>>> ${explanation || ''}`.trim(),
                last_reviewed_at: new Date().toISOString()
              }).eq('id', quiz.id);
            }
          } else {
            let msg = `🩺 <b>حالة سريرية للمراجعة والتثبيت [${quiz.course_code}]:</b>\n`;
            msg += `━━━━━━━━━━━━━━━━━━━━━\n`;
            ...
            const keyboard = {
              inline_keyboard: [
                [{ text: '💡 أظهر التشخيص وتريكة الراوند', callback_data: `reveal_med_${quiz.id}` }]
              ]
            };
  ...
  if (prefs.islamic !== false) {
    ...
    const { data: userWorship, error: worshipErr } = await supabase
    ...
    if (worshipErr) {
      console.warn('[Scheduler Worship Fetch Error]:', worshipErr.message);
      // SAFETY GUARD: Do not send worship notifications if DB is offline or restricted
      return;
    }
          ...
          if (item.day_of_week && (dayName.includes(item.day_of_week) || item.day_of_week.includes(dayName))) {
            ...
            if (diff > 0 && diff <= (item.reminder_mins_before || 60)) {
              ...
              if (!(await hasSent(key, 24 * 3600 * 1000))) {
                await markSent(key, 24 * 3600 * 1000);
                let msg = `⏰ <b>تذكير بموعد سيكشن / محاضرة قادمة:</b>\n`;
                msg += `━━━━━━━━━━━━━━━━━━━━━\n`;
                msg += `🩺 <b>المقرر:</b> [${item.course_code}] ${item.title}\n`;
                msg += `🏷️ <b>النوع:</b> ${item.type || 'سيكشن'}\n`;
                msg += `📍 <b>المكان:</b> ${item.location || 'الكلية'}\n`;
                ...
                const keyboard = {
                  inline_keyboard: [
                    [
                      { text: '✅ تم الحضور', callback_data: `ack_section_attend_${item.course_code}_${dateStr}` },
        ...
        for (const task of pendingTasks) {
          ...
          if (elapsedHours >= 2.5) {
            ...
            if (!(await hasSent(key, 6 * 3600 * 1000))) {
              ...
              const gTerms = getGenderTerms(userProf?.full_name);
  ...
  try {
    ...
    // 🎯 E. فحص وإرسال الكويزات التفاعلية المجدولة للدروس التي قرأها د. عبدالله
    try {
      const nowIso = new Date().toISOString();
      const nonMedicalCodes = ['SHARIA', 'SAHABA_SPOTLIGHT', 'QURAN_ASBAB', 'BUKHARI', 'DISCIPLINE', 'PROPHETIC_LEADERSHIP'];
      const { data: dueQuizzes } = await supabase
        ...
        .in('course_code', nonMedicalCodes)
      ...
      if (dueQuizzes && dueQuizzes.length > 0) {
        ...
        const quizKey = `admin_due_quiz_sent_${qRow.id}_${Math.floor(Date.now() / 3600000)}`;
        if (!(await hasSent(quizKey, 3600000))) {
          await markSent(quizKey, 3600000);
          let meta = {};
          ...
          } else if (Array.isArray(qRow.options)) {
            meta.options = qRow.options;
            meta.correct_index = qRow.correct_option_index || 0;
            meta.explanation = qRow.explanation || qRow.answer_and_explanation;
          }

          if (meta.options && meta.options.length >= 2) {
            const cleanTopic = qRow.topic.replace(/\[UID:\d+\]\s*/g, '').trim();
            ...
            const pollMsg = await bot.telegram.sendQuiz(
              adminChatId,
              qRow.question.substring(0, 300),
              meta.options.map(o => String(o).substring(0, 100)),
              {
                correct_option_id: Math.max(0, Math.min(meta.options.length - 1, Number(meta.correct_index || 0))),
                is_anonymous: false,
                explanation: (meta.explanation || qRow.answer_and_explanation || '').substring(0, 195)
              }
            );

            if (pollMsg && pollMsg.poll) {
              meta.poll_id = pollMsg.poll.id;
              const nextLvl = Math.max(1, (qRow.repetition_level || 0) + 1);
              const intervals = { 1: 12, 2: 24, 3: 72, 4: 168, 5: 360, 6: 720 };
              const nextHours = intervals[nextLvl] || 24;
              const nextReviewDate = new Date(Date.now() + nextHours * 3600 * 1000).toISOString();

              await supabase.from('medical_spaced_quizzes').update({
                repetition_level: nextLvl,
                next_review_at: nextReviewDate,
                is_mastered: nextLvl >= 6,
      ...
      if (!(await hasSent(discKey, 86400000))) {
        ...
        await bot.telegram.sendMessage(adminChatId, disc.formattedText, {
          ...
          reply_markup: {
            inline_keyboard: [
              ...
              [{ text: '🎯 اختبرني الآن فوراً', callback_data: `take_discipline_quiz_${disc.item.id}` }],
        ...
        if (disc.quiz) {
          try {
            ...
            await scheduleAdminSpacedQuiz(adminChatId, {
              course_code: 'DISCIPLINE',
              topic: disc.item.title,
        ...
        await bot.telegram.sendMessage(adminChatId, ayah.formattedText, {
          ...
          reply_markup: {
            inline_keyboard: [
              ...
              [{ text: '🎯 اختبرني الآن فوراً', callback_data: `take_ayah_quiz_${ayah.item.id}` }],
        ...
        await bot.telegram.sendMessage(adminChatId, sahabi.formattedText, {
          ...
          reply_markup: {
            inline_keyboard: [
              ...
              [{ text: '🎯 اختبرني الآن فوراً', callback_data: `take_sahabi_quiz_${sahabi.item.id}` }],
      ...
      if (!(await hasSent(bukhariKey, 86400000))) {
        ...
        await bot.telegram.sendMessage(adminChatId, bukhari.formattedText, {
          ...
          reply_markup: {
            inline_keyboard: [
              ...
              [{ text: '🎯 اختبرني الآن فوراً', callback_data: `take_bukhari_quiz_${bukhari.item.id}` }],
        ...
        if (bukhari.quiz) {
          try {
            ...
            await scheduleAdminSpacedQuiz(adminChatId, {
              course_code: 'BUKHARI',
              topic: bukhari.item.hadith_title,
        ...
        await bot.telegram.sendMessage(adminChatId, header + capsule.formattedText, {
          ...
          reply_markup: {
            inline_keyboard: [
              ...
              [{ text: '🎯 اختبرني الآن فوراً', callback_data: `take_sharia_quiz_${capsule.id}` }],
