...
import { supabase, getAllRegisteredUsers, ADMIN_CHAT_ID } from './supabase.js';
...
async function hasSent(key, ttlMs = 86400000) {
  try {
    const tracker = await getTrackerFromDb();
    if (!tracker || !tracker[key]) return false;
    const sentTimestamp = Number(tracker[key]);
    if (isNaN(sentTimestamp)) return false;
...
async function markSent(key, ttlMs = 86400000) {
  try {
    const tracker = await getTrackerFromDb();
    ...
    const pruneThreshold = Date.now() - (7 * 86400000);
    ...
    console.error('[NotificationTracker DB Save Error]:', e.message);
  }
}
...
async function runUserCycle(bot, user) {
  ...
  const userName = user.full_name || 'يا دكتور';
  ...
  try {
    ...
    if (dueAppts && dueAppts.length > 0) {
      for (const appt of dueAppts) {
        ...
        const isUserMatch = !appt.notes?.includes('usr:') || appt.notes.includes(`usr:${chatId}`) || (appt.telegram_id && Number(appt.telegram_id) === Number(chatId));
