...
export async function hasSent(key, ttlMs = 86400000) {
  try {
    ...
    // Check exact key
    let sentTimestamp = tracker[key] ? Number(tracker[key]) : null;
    
    // Check stripped base key (e.g. without _chatId suffix) for legacy compatibility
    if (!sentTimestamp) {
      const baseKey = key.replace(/_\d+$/, '');
      if (tracker[baseKey]) {
        sentTimestamp = Number(tracker[baseKey]);
      }
    }
...
export async function markSent(key, ttlMs = 86400000) {
  try {
    ...
    // Also mark base key for backward compatibility
    const baseKey = key.replace(/_\d+$/, '');
    tracker[baseKey] = now;
