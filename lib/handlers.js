...
export function registerHandlers(bot) {
  ...
  bot.use(async (ctx, next) => {
    ...
    if (!profile) {
      ...
      // Check if text is a valid name declaration: e.g. "أنا اسمي د. محمد أحمد", "انا اسمي د. أحمد", "اسمي محمود", "د. أحمد محمود"
      const nameMatch = text ? text.match(/^(?:أنا\s+اسمي|انا\s+اسمي|اسمي|أنا\s+دكتور|انا\s+دكتور|دكتور|د\.|د\/)\s*(.+)$/i) : null;
      let candidateName = nameMatch ? nameMatch[1].trim() : null;

      if (candidateName) {
        candidateName = candidateName.replace(/^[\(\[\{]/, '').replace(/[\)\]\}]$/, '').replace(/^["'«»]/, '').replace(/["'«»]$/, '').trim();
      }

      if (isRegistering && candidateName && candidateName.length >= 2 && !/^\d+$/.test(candidateName)) {

        // User supplied their full name correctly
        const cleanName = candidateName.startsWith('د.') || candidateName.startsWith('د/') ? candidateName : `د. ${candidateName}`;
        ...
        // Notify user with clear next steps, commands tip, and direct configuration buttons
        let regMsg = `🎉 <b>أهلاً بك يا ${newProfile.full_name} في منظومة الطبيب الذكية (Doctor OS)! 🩺</b>\n`;
        regMsg += `━━━━━━━━━━━━━━━━━━━━━\n`;
        regMsg += `✨ <b>تم تفعيل حسابك وفترتك التجريبية المجانية (3 أيام) بنجاح! 🎁</b>\n\n`;
        regMsg += `⚡ <b>المنظومة مصممة لتكون مخصصة ومفصلة على مقاسك 100%:</b>\n`;
        regMsg += `1️⃣ <b>تخصيص الأقسام:</b> إظهار أو إخفاء أي قسم (مثل الجيم، صناعة المحتوى، أو القسم الإسلامي).\n`;
        regMsg += `2️⃣ <b>الفرقة والموديولات:</b> اختيار فرقتك الدراسية (سنة 1، 2، 3، 4...) لضبط موديولاتك وسكاشنك.\n\n`;
        regMsg += `💡 <b>نصيحة سريعة للتحكم الكامل بالأوامر:</b>\n`;
        regMsg += `يمكنك في أي وقت كتابة علامة <code>/</code> في مكان الكتابة لتظهر لك كل الأوامر الرسمية بالعربي كاملة (مثل فتح القائمة <code>/menu</code>، الكويزات <code>/quiz</code>، الموديولات <code>/academic</code>، تخصيص الأقسام <code>/settings</code>، أو تعديل اسمك <code>/rename</code>).\n\n`;
        regMsg += `👇 <i>اختر ما تريد ضبطه الآن أو افتح القائمة الرئيسية:</i>`;

        const postRegKeyboard = {
          inline_keyboard: [
            [{ text: '⚙️ تخصيص وتفعيل الأقسام والاهتمامات', callback_data: 'menu_settings' }],
            [{ text: '🎓 ضبط الفرقة والتيرم والموديولات', callback_data: 'menu_academic_config' }],
            [{ text: '🚀 فتح القائمة الرئيسية', callback_data: 'menu_main' }]
          ]
        };

        await ctx.reply(regMsg, { parse_mode: 'HTML', reply_markup: postRegKeyboard });
      ...
      // If user hasn't started registration or sent an invalid casual text, guide them clearly
      ...
      let promptMsg = `🩺 <b>أهلاً بك في منظومة الطبيب الذكية (Doctor OS)! 🌟</b>\n`;
      promptMsg += `━━━━━━━━━━━━━━━━━━━━━\n`;
      promptMsg += `المساعد الذكي الفائق لطلاب كليات الطب لتنظيم المذاكرة، السكاشن، بنك الكويزات، ممارسة الإنجليزية، ومتابعة المصاريف والمهام بالفويس والذكاء الاصطناعي.\n\n`;
      promptMsg += `🚀 <b>خطوة تفعيل حسابك وبدء تجربتك المجانية (3 أيام):</b>\n`;
      promptMsg += `لتسجيل حسابك، يرجى كتابة اسمك مسبوقاً بـ <b>(أنا اسمي)</b> أو <b>(د.)</b> في رسالة، مثل:\n\n`;
      promptMsg += `👉 <code>أنا اسمي د. محمد أحمد</code>\n`;
      promptMsg += `👉 <code>أنا اسمي د. أحمد محمود</code>\n`;
      promptMsg += `👉 <code>د. سارة إبراهيم</code>\n\n`;
      promptMsg += `✍️ <i>اكتب اسمك الآن في رسالة بالأسفل للبدء فوراً! 👇</i>`;
  ...
  bot.telegram.setMyCommands([
    ...
    { command: 'plan', description: '📋 خطة المذاكرة وبقية اليوم' },
    ...
    { command: 'commands', description: '🧭 دليل الأوامر السريعة' },
  ...
  function getCommandsGuideContent(profile) {
    ...
    const sem = profile?.semester || 'الترم الأول';

    let msg = `🧭 <b>دليل الأوامر السريعة والتحكم في المنظومة:</b>\n`;
    msg += `━━━━━━━━━━━━━━━━━━━━━\n`;
    msg += `👤 <b>الطالب:</b> <b>${studentName}</b> (${year} - ${sem})\n\n`;

    msg += `🏷️ <b>تعديل الاسم:</b>\n`;
    msg += `• اكتب <code>/rename اسمك الجديد</code> (مثال: <code>/rename د. محمد أحمد</code>)\n`;
    msg += `• أو اضغط على زر <b>[ 🏷️ تعديل اسم الحساب ]</b> بالأسفل.\n\n`;

    msg += `🎓 <b>الفرقة والموديولات:</b>\n`;
    msg += `• اكتب <code>/academic</code> لتبديل فرقتك أو التيرم أو إضافة موديول جديد.\n\n`;

    msg += `⚙️ <b>تخصيص وإخفاء الأقسام:</b>\n`;
    msg += `• اكتب <code>/settings</code> للتحكم في الأقسام والتذكيرات المفعلة والمعطلة.\n\n`;

    msg += `🩺 <b>الكويزات والمذاكرة:</b>\n`;
    msg += `• اكتب <code>/quiz</code> لتوليد حالة سريرية وكويز من موديولاتك النشطة.\n`;
    msg += `• أرسل تسجيلاً صوتياً أو نصاً بمذاكرتك لتوثيقها فوراً.\n\n`;

    msg += `🗣️ <b>فلاش كاردز الإنجليزية:</b>\n`;
    msg += `• اكتب <code>/english</code> أو تحدث بالإنجليزي مع المدرب الصوتي.\n\n`;

    msg += `📊 <b>القوائم والإحصائيات:</b>\n`;
    msg += `• <code>/menu</code> — فتح القائمة الرئيسية.\n`;
    msg += `• <code>/stats</code> — ملخص الإنجاز والتحصيل.\n`;
    msg += `• <code>/support</code> — تفاصيل الاشتراك والدعم الفني.\n`;

    const keyboard = {
      inline_keyboard: [
        [
          { text: '🏷️ تعديل اسم الحساب', callback_data: 'edit_profile_name' },
          { text: '🎓 الفرقة والموديولات', callback_data: 'menu_academic_config' }
        ],
        [
          { text: '⚙️ تخصيص الأقسام', callback_data: 'menu_settings' },
        ...
        [
          ...
          { text: '📊 إحصائيات المذاكرة', callback_data: 'menu_stats' }
        ],
        [
  ...
  async function sendDynamicMainMenu(ctx) {
    const fromId = ctx.from?.id;
    const profile = await getUserProfile(fromId);
    const name = profile?.full_name || ctx.from?.first_name || 'دكتور';
    const prefs = profile?.preferences || DEFAULT_USER_PREFERENCES;
    const year = profile?.academic_year || 'الفرقة الرابعة';
  ...
  bot.command(['commands', 'help', 'guide', 'اوامر', 'مساعدة'], async (ctx) => {
    const fromId = ctx.from?.id;
    const profile = await getUserProfile(fromId);
  ...
  bot.command(['now', 'plan', 'next', 'what_to_do', 'اعمل_ايه', 'توجيه', 'تايه', 'اعمل_ايه_دلوقتي', 'خطتي'], async (ctx) => {
  ...
  bot.command(['rename', 'setname', 'تسمية', 'اسم'], async (ctx) => {
    ...
    if (Number(fromId) === ADMIN_CHAT_ID && parts.length >= 3 && /^\d+$/.test(parts[1])) {
      ...
      const newName = rawName.startsWith('د.') || rawName.startsWith('د/') ? rawName : `د. ${rawName}`;
    ...
    if (parts.length >= 2) {
      let rawName = parts.slice(1).join(' ').replace(/^(?:أنا\s+اسمي|انا\s+اسمي|اسمي)\s*/i, '').trim();
      if (rawName.length < 2) {
  ...
  bot.on('text', async (ctx) => {
    ...
    try {
      ...
      if (session?.state === 'waiting_new_name') {
        await ctx.deleteMessage(waitMsg.message_id).catch(() => {});
        await setUserSession(fromId, { state: 'idle' });
        let rawName = text.replace(/^(?:أنا\s+اسمي|انا\s+اسمي|اسمي)\s*/i, '').trim();
        const newName = rawName.startsWith('د.') || rawName.startsWith('د/') ? rawName : `د. ${rawName}`;
  ...
  async function executeParsedLifeActions(ctx, result, fromId) {
    ...
    if (data.fasting_worship && (data.fasting_worship.fasting_type || data.fasting_worship.sunan_rawatib_count != null || data.fasting_worship.adhkar_morning || data.fasting_worship.adhkar_evening || data.fasting_worship.duha_prayer_done || data.fasting_worship.witr_prayer_done)) {
      ...
      try {
        ...
        let addedSunan = 0;
        if (fw.sunan_rawatib_count != null) {
          if (typeof fw.sunan_rawatib_count === 'number') addedSunan = isNaN(fw.sunan_rawatib_count) ? 0 : fw.sunan_rawatib_count;
          else {
            const m = String(fw.sunan_rawatib_count).match(/\d+/);
            addedSunan = m ? parseInt(m[0], 10) : 2;
          }
        }

        const totalSunan = Math.min(12, (Number(existing?.sunan_rawatib_count || 0) + addedSunan));
        ...
        insertedSummary.push(`🌙 <b>الصيام والسنن والأذكار:</b>\n   └ ${payload.fasting_type ? `✨ ${payload.fasting_type} | ` : ''}🕌 سنن: ${payload.sunan_rawatib_count} / 12 ركعة | 🌅 صباح: ${payload.adhkar_morning ? '✅' : '—'} | 🌇 مساء: ${payload.adhkar_evening ? '✅' : '—'}`);
      } catch (e) {
    ...
    if (data.prayer_habits && Object.keys(data.prayer_habits).length > 0) {
      ...
      try {
        ...
        const existingFajr = (existingHabits?.fajr && existingHabits.fajr !== 'لم يُسجل') ? existingHabits.fajr : null;
        const existingDhuhr = (existingHabits?.dhuhr && existingHabits.dhuhr !== 'لم يُسجل') ? existingHabits.dhuhr : null;
        const existingAsr = (existingHabits?.asr && existingHabits.asr !== 'لم يُسجل') ? existingHabits.asr : null;
        const existingMaghrib = (existingHabits?.maghrib && existingHabits.maghrib !== 'لم يُسجل') ? existingHabits.maghrib : null;
        const existingIsha = (existingHabits?.isha && existingHabits.isha !== 'لم يُسجل') ? existingHabits.isha : null;

        // Parse Sunan count accurately
        let addedSunan = 0;
        if (p.sunan_rawatib != null) {
          if (typeof p.sunan_rawatib === 'number') {
            addedSunan = isNaN(p.sunan_rawatib) ? 0 : p.sunan_rawatib;
          } else if (typeof p.sunan_rawatib === 'string') {
            const s = p.sunan_rawatib.toLowerCase();
            if (s.includes('4') || s.includes('أربع') || s.includes('اربع')) addedSunan = 4;
            else if (s.includes('2') || s.includes('ركعتين') || s.includes('سنة') || s.includes('الفجر') || s.includes('المغرب') || s.includes('العشاء')) addedSunan = 2;
            else {
              const m = s.match(/\d+/);
              addedSunan = m ? parseInt(m[0], 10) : 2;
            }
        ...
        const totalSunan = Math.min(12, (Number(existingHabits?.sunan_rawatib || 0) + addedSunan));

        const payload = {
          date: todayDate,
          fajr: p.fajr || existingFajr || sessPrayers.fajr || 'لم يُسجل',
          dhuhr: p.dhuhr || existingDhuhr || sessPrayers.dhuhr || 'لم يُسجل',
          asr: p.asr || existingAsr || sessPrayers.asr || 'لم يُسجل',
          maghrib: p.maghrib || existingMaghrib || sessPrayers.maghrib || 'لم يُسجل',
          isha: p.isha || existingIsha || sessPrayers.isha || 'لم يُسجل',
        ...
        if (addedSunan > 0 || p.adhkar_morning || p.adhkar_evening) {
          const { data: existingFw } = await supabase.from('fasting_and_worship_logs').select('*').eq('date', todayDate).maybeSingle();
          const fwSunan = Math.min(12, Number(existingFw?.sunan_rawatib_count || 0) + addedSunan);
          await supabase.from('fasting_and_worship_logs').upsert({
            date: todayDate,
            sunan_rawatib_count: fwSunan,
            adhkar_morning: p.adhkar_morning != null ? p.adhkar_morning : (existingFw?.adhkar_morning || false),
            adhkar_evening: p.adhkar_evening != null ? p.adhkar_evening : (existingFw?.adhkar_evening || false),
            notes: `[usr:${fromId}]`
          }, { onConflict: 'date' });
        }
        ...
        if (p.fajr) prayerListText.push(`الفجر (${p.fajr})`);
        if (p.dhuhr) prayerListText.push(`الظهر (${p.dhuhr})`);
        if (p.asr) prayerListText.push(`العصر (${p.asr})`);
        if (p.maghrib) prayerListText.push(`المغرب (${p.maghrib})`);
        if (p.isha) prayerListText.push(`العشاء (${p.isha})`);
        ...
        if (addedSunan > 0) {
          insertedSummary.push(`🕌 <b>السنن الرواتب:</b> تم توثيق (${addedSunan} ركعات) بنجاح ➔ إجمالي اليوم: (${totalSunan}/12 ركعة)`);
        }
