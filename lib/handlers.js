...
export function registerHandlers(bot) {
  ...
  bot.use(async (ctx, next) => {
    ...
    if (!profile.is_active) {
      ...
      // Show Paywall
      let paywallMsg = `🔒 <b>عفواً يا ${profile.full_name}، انتهت فترتك التجريبية! 🎯</b>\n`;
      paywallMsg += `━━━━━━━━━━━━━━━━━━━━━\n`;
      paywallMsg += `للاستمرار في استخدام المنظومة، يرجى تحويل رسوم الاشتراك الشهري (<b>30 ج.م فقط شهرياً</b>) عبر أحد الحسابات التالية:\n\n`;
      paywallMsg += `📱 <b>فودافون كاش:</b> <code>01000000000</code>\n`;
      paywallMsg += `💳 <b>إنستا باي (InstaPay):</b> <code>abdallah@instapay</code>\n\n`;
      paywallMsg += `📸 <b>بعد التحويل:</b> أرسل سكرين شوت الإيصال هنا في الشات وسيتم تفعيل حسابك فوراً! 🚀`;

      return ctx.reply(paywallMsg, { parse_mode: 'HTML' });
    }
  ...
  bot.telegram.setChatMenuButton({
    menu_button: {
      type: 'web_app',
      text: '📱 لوحة التحكم',
      web_app: { url: 'https://abdallah-pro.vercel.app/api/dashboard' }
    }
  }).catch(() => {});
  ...
  bot.command(['start', 'menu', 'help'], async (ctx) => {
    const name = ctx.userProfile?.full_name || ctx.from?.first_name || 'د. عبدالله';
    ...
    const keyboard = {
      inline_keyboard: [
        [
          { text: '📱 فتح لوحة التحكم التفاعلية (Web App)', web_app: { url: 'https://abdallah-pro.vercel.app/api/dashboard' } }
        ],
        [
          { text: '⚡ نبضة المجد والانضباط واليقين 👑', callback_data: 'get_another_mindset_pulse' }
        ],
        [
  ...
  bot.on(['voice', 'audio'], async (ctx) => {
    ...
    try {
      ...
      if (isEnglishMode) {
        ...
        let msg = `🗣️ <b>English Coach Feedback:</b>\n━━━━━━━━━━━━━━━━━━━━━\n💬 <b>Reply:</b>\n${coachResult.conversational_reply || 'Well said, Dr. Abdallah!'}\n\n`;
      ...
      const studentName = ctx.userProfile?.full_name || 'د. عبدالله';
  ...
  bot.on('text', async (ctx) => {
    ...
    try {
      ...
      if (isEnglishMode) {
        ...
        let msg = `🗣️ <b>English Coach Feedback:</b>\n\n💬 ${coachResult.conversational_reply || 'Great message!'}\n\n`;
        if (coachResult.corrections?.length > 0) {
          msg += `🛠️ <b>Corrections:</b>\n`;
          coachResult.corrections.forEach(c => msg += `• <s>${c.original}</s> ➔ <b>${c.corrected}</b> (${c.reason})\n`);
        }
  ...
  async function executeParsedLifeActions(ctx, result, fromId) {
      ...
      for (const f of data.finance) {
        try {
          ...
          if (amt > 0) {
            ...
            const description = f.description || (type === 'إيراد' ? 'استلام نقدية' : 'مصروف شخصي');
            ...
            if (row?.id) {
              ...
              financeItems.push({ id: row.id, type: type, amount: amt, method: method, category: category, description: description });
            }
            insertedSummary.push(`💵 <b>${type}:</b> <b>${formatEgp(amt)}</b>\n   └ 🏷️ <b>البند:</b> ${description || category} | 💳 <b>الخزنة:</b> ${method}`);
          }
        } catch (e) {
          ...
          if (taskTitle) {
            ...
            if (tk.reminder_after_minutes || tk.reminder_mins) {
              ...
              await supabase.from('appointments_and_reminders').insert({
                ...
                notes: `مجدولة تلقائياً بعد ${mins} دقيقة`,
                date: todayDate,
                is_notified: false,
                is_completed: false
              }).catch(() => {});
      ...
      for (const q of data.quran) {
        try {
          if (q.surah_name) {
            ...
            insertedSummary.push(`📖 <b>القرآن الكريم:</b> <b>سورة ${q.surah_name}</b>\n   └ 📑 <b>الكمية:</b> ${q.pages_count || 1} صفحة (${q.session_type || 'مراجعة تثبيت'}) | ⭐ الإتقان: ${q.quality_rating || 5}/5`);
          }
        } catch (e) {
    ...
    if (Array.isArray(data.study) && data.study.length > 0) {
      for (const s of data.study) {
        try {
          if (s.topic || s.course_code) {
            ...
            if (row?.id) {
              recordedUndoItems.push({ table: 'study_sessions', id: row.id, summary: `🩺 مذاكرة [${s.course_code || 'CAD402'}] ${s.topic} (${durText})` });
            }
            ...
            if (s.was_rescheduled) {
              insertedSummary.push(`🔄 <b>إعادة جدولة مذاكرة [${s.course_code || 'CAD402'}]:</b> ${s.topic}\n   └ ⏱️ <b>المدة:</b> <b>${durText}</b>${s.reschedule_reason ? ` (السبب: ${s.reschedule_reason})` : ''}`);
            } else {
      ...
      try {
        const { data: allTodayStudy } = await supabase.from('study_sessions').select('duration_minutes').eq('date', todayDate);
        todayStudyTotalMinutes = (allTodayStudy || []).reduce((acc, curr) => acc + Number(curr.duration_minutes || 0), 0);
      } catch (e) {}
    }
      ...
      for (const g of data.fitness_gym) {
        try {
          const { data: row } = await supabase.from('fitness_gym_logs').insert({
            ...
            muscle_groups: g.muscle_groups || null,
          ...
          insertedSummary.push(`🏋️‍♂️ <b>تمرين الجيم:</b> <b>${g.workout_type || 'حديد وتمارين مقاومة'}</b>\n   └ ⏱️ <b>المدة:</b> ${g.duration_minutes || 45} دقيقة${g.muscle_groups ? ` | 💪 <b>العضلات:</b> ${g.muscle_groups}` : ''}${g.protein_grams ? ` | 🥩 <b>البروتين:</b> ${g.protein_grams} جم` : ''}`);
        } catch (e) {
        ...
        try {
          if (w.project_name || w.task_description) {
            const { data: row } = await supabase.from('work_projects').insert({
              ...
              task_description: w.task_description || 'مهمة عمل',
        ...
        try {
          if (sd.title) {
            const { data: row } = await supabase.from('self_development_books').insert({
              ...
              actionable_habits: sd.actionable_habits || null,
              date: todayDate
            }).select('id').maybeSingle();
    ...
    if (data.clinical_case && (data.clinical_case.title || data.clinical_case.chief_complaint)) {
      ...
      try {
        const { data: row } = await supabase.from('clinical_cases').insert({
          ...
          doctor_pearls: cc.doctor_pearls || null,
    ...
    try {
      ...
      if (Array.isArray(undoData.financeReversions)) {
        for (const rev of undoData.financeReversions) {
          try {
            await updateLiquidity(rev.method, rev.amountChange);
          } catch (e) {
  ...
  bot.action(/^setpm_([^_]+)_(cash|voda|insta|bank)$/, async (ctx) => {
    ...
    try {
      ...
      await updateLiquidity(oldMethod, -1 * factor * amt);
      // 2. Apply to new wallet
      await updateLiquidity(newMethod, factor * amt);
  ...
  bot.action(/^ack_appt_done_(.+)$/, async (ctx) => {
    ...
    return ctx.editMessageText('✅ <b>عاش يا دكتور عبدالله! تم إنجاز وتوثيق الموعد بنجاح 🎯</b>', { parse_mode: 'HTML', reply_markup: keyboard }).catch(() => {});
  });
  ...
  bot.action('get_another_mindset_pulse', async (ctx) => {
    await ctx.answerCbQuery('⚡ نبضة ملهمة جديدة!');
    await sendMindsetPulse(bot, ctx.chat.id, true);
  });
}
