...
export function registerHandlers(bot) {
  ...
  bot.use(async (ctx, next) => {
    ...
    let candidateName = nameMatch ? nameMatch[1].trim() : (isRegistering && text && !text.startsWith('/') && !/^\d+$/.test(text) && text.length >= 2 && text.length <= 40 ? text : null);
    ...
    if (candidateName && candidateName.length >= 2 && !/^\d+$/.test(candidateName) && !candidateName.startsWith('/')) {
      ...
      let regMsg = `🎉 <b>أهلاً بك يا ${cleanName} في منظومة الطبيب الذكية (Doctor OS)! 🩺</b>\n`;
      regMsg += `━━━━━━━━━━━━━━━━━━━━━\n`;
      regMsg += `✨ <b>تم حفظ اسمك وفترتك التجريبية المجانية (3 أيام) مفعّلة بنجاح! 🎁</b>\n\n`;
    ...
    // If completely unregistered, prompt them to write their name
    if (!profile) {
      ...
      promptMsg += `🚀 <b>فترتك التجريبية المجانية (3 أيام) نشطة الآن! 🎁</b>\n`;
      promptMsg += `لتسجيل اسمك في حسابك وظهوره على كويزاتك ولوحة التحكم، فقط أرسل اسمك في رسالة بالأسفل:\n\n`;
      promptMsg += `👉 <code>محمد أحمد</code> أو <code>د. محمد أحمد</code> أو <code>سارة إبراهيم</code>\n\n`;
      promptMsg += `✍️ <i>اكتب اسمك الآن للبدء فوراً! 👇</i>`;
