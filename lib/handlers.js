...
export function registerHandlers(bot) {
  ...
  bot.telegram.setMyCommands([
    ...
    { command: 'quiz', description: '🩺 كويز سريري وحالة إكلينيكية' },
  ...
  bot.command(['quiz', 'كويز', 'سؤال'], async (ctx) => {
    const fromId = ctx.from?.id;
    const waitMsg = await ctx.reply('🧠 <i>جاري تحضير حالة سريرية وسؤال تفاعلي من موديولاتك النشطة...</i>', { parse_mode: 'HTML' });
    try {
      const aiKeys = await getStoredAiKeys();
      const profile = await getUserProfile(fromId);
      const studentName = profile?.full_name || ctx.from?.first_name || 'دكتور';
      const activeCourses = await getUserActiveCourses(fromId);
      const randomCourse = activeCourses && activeCourses.length > 0
        ? activeCourses[Math.floor(Math.random() * activeCourses.length)]
        : { code: 'MED', title: 'Medical Pearl' };

      const quiz = await generateMedicalQuiz(`[${randomCourse.code}] ${randomCourse.title}`, 'High-Yield Clinical Pearls & USMLE Scenarios', aiKeys, studentName);
      await ctx.deleteMessage(waitMsg.message_id).catch(() => {});

      let msg = `🩺 <b>حالة إكلينيكية تفاعلية [${randomCourse.code}]:</b>\n━━━━━━━━━━━━━━━━━━━━━\n📋 <b>السيناريو:</b>\n${quiz.case_scenario}\n\n❓ <b>السؤال:</b>\n<b>${quiz.question}</b>\n`;
