  ...
  async function executeParsedLifeActions(ctx, result, fromId) {
    ...
    if (data.prayer_habits && Object.keys(data.prayer_habits).length > 0) {
      ...
      try {
        ...
        const payload = {
          date: todayDate,
          fajr: (canLogPrayer('fajr') ? p.fajr : null) || (isTodayDate && !canLogPrayer('fajr') ? 'لم يُسجل' : (existingFajr || sessPrayers.fajr || 'لم يُسجل')),
          dhuhr: (canLogPrayer('dhuhr') ? p.dhuhr : null) || (isTodayDate && !canLogPrayer('dhuhr') ? 'لم يُسجل' : (existingDhuhr || sessPrayers.dhuhr || 'لم يُسجل')),
          asr: (canLogPrayer('asr') ? p.asr : null) || (isTodayDate && !canLogPrayer('asr') ? 'لم يُسجل' : (existingAsr || sessPrayers.asr || 'لم يُسجل')),
          maghrib: (canLogPrayer('maghrib') ? p.maghrib : null) || (isTodayDate && !canLogPrayer('maghrib') ? 'لم يُسجل' : (existingMaghrib || sessPrayers.maghrib || 'لم يُسجل')),
          isha: (canLogPrayer('isha') ? p.isha : null) || (isTodayDate && !canLogPrayer('isha') ? 'لم يُسجل' : (existingIsha || sessPrayers.isha || 'لم يُسجل')),
