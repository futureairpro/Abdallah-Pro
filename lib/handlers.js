...
export function registerHandlers(bot) {
  ...
  bot.use(async (ctx, next) => {
    ...
    const isRegistering = !profile || profile.full_name === 'طالب زميل' || session?.state === 'registering_name' || session?.state === 'waiting_new_name';
    const nameMatch = text ? text.match(/^(?:أنا\s+اسمي|انا\s+اسمي|اسمي|أنا\s+دكتور|انا\s+دكتور|دكتور|د\.|د\/)\s*(.+)$/i) : null;
    let candidateName = nameMatch ? nameMatch[1].trim() : (isRegistering && text && !text.startsWith('/') && text.split(/\s+/).length >= 2 ? text : null);

    if (candidateName) {
      candidateName = candidateName.replace(/^[\(\[\{]/, '').replace(/[\)\]\}]$/, '').replace(/^["'«»]/, '').replace(/["'«»]$/, '').trim();
    }

    if (candidateName && candidateName.length >= 2 && !/^\d+$/.test(candidateName) && !candidateName.startsWith('/')) {
      // User supplied their full name correctly
      const cleanName = formatDoctorName(candidateName);
      ...
      regMsg += `━━━━━━━━━━━━━━━━━━━━━\n`;
      regMsg += `✨ <b>تم حفظ اسمك وتفعيل فترتك التجريبية المجانية (3 أيام) بنجاح! 🎁</b>\n\n`;
    ...
    // Refresh profile in case it was modified
    profile = await getUserProfile(fromId);
  ...
  bot.command(['rename', 'name', 'اسم'], async (ctx) => {
    ...
    return ctx.reply(successMsg, {
      ...
      reply_markup: {
        inline_keyboard: [
          [{ text: '🚀 فتح القائمة الرئيسية', callback_data: 'menu_main' }],
          [{ text: '📱 لوحة التحكم', web_app: { url: `https://abdallah-pro.vercel.app/?telegram_id=${fromId}` } }]
        ]
      }
    });
  });
