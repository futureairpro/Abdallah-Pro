    ...
    if (data.prayer_habits && Object.keys(data.prayer_habits).length > 0) {
      ...
      try {
        ...
        const payload = {
          ...
          daily_reflection: `[usr:${fromId}] ${p.daily_reflection || existing?.daily_reflection || ''}`.trim()
        };

        await supabase.from('prayers_and_habits').upsert(payload, { onConflict: 'date' });

        // Update user's personal session with prayer status
        if (fromId) {
          try {
            const userSess = await getUserSession(fromId);
            const curPrayers = userSess?.data?.prayers_today || {};
            if (p.fajr) curPrayers.fajr = p.fajr;
            if (p.dhuhr) curPrayers.dhuhr = p.dhuhr;
            if (p.asr) curPrayers.asr = p.asr;
            if (p.maghrib) curPrayers.maghrib = p.maghrib;
            if (p.isha) curPrayers.isha = p.isha;
