...
export function registerHandlers(bot) {
  ...
  bot.use(async (ctx, next) => {
    ...
    if (!profile) {
      ...
      // Check if text is a valid name declaration: e.g. "أنا اسمي علي أشرف", "انا اسمي د. أحمد", "اسمي محمود", "د. علي أشرف"
      ...
      if (isRegistering && candidateName && candidateName.length >= 2 && !/^\d+$/.test(candidateName)) {
        ...
        // Notify user with clear next steps and direct configuration buttons
      ...
      promptMsg += `👉 <code>أنا اسمي علي أشرف</code>\n`;
  ...
  function buildDynamicMenuKeyboard(profile) {
    ...
    // Row 8: Support & Sandbox
    rows.push([
      { text: '💬 الدعم الفني والاشتراك', url: 'https://t.me/Dr31327' },
      { text: '🧪 وضع التجربة (Sandbox)', callback_data: 'menu_sandbox' }
    ]);
  ...
  function getCommandsGuideContent(profile) {
    ...
    msg += `• اكتب <code>/rename اسمك الجديد</code> (مثال: <code>/rename د. علي أشرف</code>)\n`;
  ...
  async function sendDynamicMainMenu(ctx) {
    ...
    const keyboard = buildDynamicMenuKeyboard(profile);
  ...
  bot.command(['rename', 'setname', 'تسمية', 'اسم'], async (ctx) => {
    ...
    return ctx.reply(`✍️ <b>تعديل اسمك في المنظومة:</b>\nيرجى كتابة اسمك الجديد في رسالة الآن (مثال: <code>د. علي أشرف</code> أو <code>أنا اسمي علي أشرف</code>):`, {
      parse_mode: 'HTML',
