  ...
  async function executeParsedLifeActions(ctx, result, fromId) {
      ...
      for (const card of data.english_flashcards) {
        try {
          if (card.term_or_sentence) {
            ...
            insertedSummary.push(`🗣️ <b>فلاش كارد إنجليزي:</b> <b>${card.term_or_sentence}</b>\n   └ 🇪🇬 <b>المعنى:</b> ${card.egyptian_translation || 'مفردة جديدة'}${card.example_sentence ? `\n   └ 💬 <i>"${card.example_sentence}"</i>` : ''}`);
          }
        } catch (e) {
      ...
      for (const att of data.attendance) {
        try {
          if (att.session_title || att.course_code) {
            ...
            if (row?.id) {
              recordedUndoItems.push({ table: 'attendance_logs', id: row.id, summary: `سيكشن [${att.course_code || 'CAD402'}] ${att.session_title}` });
            }
            const icon = att.status === 'حضور' ? '✅' : '⚠️';
            insertedSummary.push(`${icon} <b>سجل السكاشن [${att.course_code || 'CAD402'}]:</b> ${att.session_title}\n   └ 📌 <b>الحالة:</b> ${att.status}${att.reason ? ` (السبب: ${att.reason})` : ''}`);
          }
        } catch (e) {
    ...
    if (data.mental_wellness && (data.mental_wellness.venting_content || data.mental_wellness.emotional_state)) {
      ...
      try {
        ...
        if (row?.id) {
          recordedUndoItems.push({ table: 'mental_wellness_logs', id: row.id, summary: `🧠 سجل الاتزان النفسي` });
        }
        insertedSummary.push(`🧠 <b>سجل الاتزان النفسي:</b> المزاج ⭐ <b>${mw.mood_rating || 4}/5</b> (${mw.stress_level || 'معتدل'})\n   └ 💬 <b>المشاعر:</b> ${mw.emotional_state || 'فضفضة وتفريغ'}`);
      } catch (e) {
      ...
      for (const f of data.finance) {
        try {
          ...
          if (amt > 0) {
            ...
            insertedSummary.push(`💵 <b>${type}:</b> <b>${formatEgp(amt)}</b>\n   └ 🏷️ <b>البند:</b> ${f.description || f.category || 'عام'} | 💳 <b>الخزنة:</b> ${method}`);
          }
        } catch (e) {
      ...
      for (const a of data.appointments) {
        try {
          if (a.title && a.due_datetime) {
            ...
            if (row?.id) {
              recordedUndoItems.push({ table: 'appointments_and_reminders', id: row.id, summary: `⏰ موعد: ${a.title}` });
            }
            insertedSummary.push(`⏰ <b>موعد مجدول:</b> <b>${a.title}</b>\n   └ 📅 <b>التوقيت:</b> ${a.due_datetime.replace('T', ' ')}`);
          }
        } catch (e) {
      ...
      for (const tk of data.tasks) {
        try {
          if (tk.title) {
            ...
            if (row?.id) {
              recordedUndoItems.push({ table: 'daily_tasks', id: row.id, summary: `🎯 مهمة: ${tk.title}` });
            }
            insertedSummary.push(`🎯 <b>مهمة مسجلة:</b> <b>${tk.title}</b>\n   └ ⚡ <b>الأولوية:</b> ${tk.priority || 'متوسطة'}${tk.target_duration_mins ? ` | ⏱️ <b>المدة المستهدفة:</b> ${tk.target_duration_mins} دقيقة` : ''}`);
          }
        } catch (e) {
