  ...
  async function executeParsedLifeActions(ctx, result, fromId) {
    ...
    if (Array.isArray(data.fitness_gym) && data.fitness_gym.length > 0) {
      for (const g of data.fitness_gym) {
        try {
          ...
          insertedSummary.push(`🏋️‍♂️ <b>تمرين الجيم:</b> ${g.workout_type || 'تمرين عام'} (${g.duration_minutes || 45} دقيقة)`);
        } catch (e) {
    ...
    if (Array.isArray(data.content_creation) && data.content_creation.length > 0) {
      for (const c of data.content_creation) {
        try {
          if (c.title) {
            await supabase.from('content_creation').insert({
              title: c.title,
            ...
            insertedSummary.push(`🎬 <b>صناعة المحتوى:</b> ${c.title} (${c.platform || 'يوتيوب'})`);
          }
        } catch (e) {
      ...
      for (const w of data.work_projects) {
        try {
          if (w.project_name || w.task_description) {
            await supabase.from('work_projects').insert({
            ...
            insertedSummary.push(`💼 <b>مشروع عمل:</b> [${w.project_name || 'بيزنس'}] ${w.task_description}`);
          }
        } catch (e) {
    ...
    if (Array.isArray(data.self_development) && data.self_development.length > 0) {
      for (const sd of data.self_development) {
        try {
          if (sd.title) {
            await supabase.from('self_development_books').insert({
              title: sd.title,
            ...
            insertedSummary.push(`🚀 <b>تطوير ذات / قراءة:</b> ${sd.title}`);
          }
        } catch (e) {
    ...
    if (data.clinical_case && (data.clinical_case.title || data.clinical_case.chief_complaint)) {
      ...
      try {
        await supabase.from('clinical_cases').insert({
        ...
        insertedSummary.push(`🏥 <b>حالة سريرية:</b> [${cc.course_code || 'CAD402'}] ${cc.title || 'حالة راوند'}`);
      } catch (e) {
