...
export function registerHandlers(bot) {
  ...
  function buildDynamicMenuKeyboard(profile) {
    ...
    // Row 7: Settings & Customization + Support
    rows.push([
    ...
    // Row 8: Sandbox Mode
    rows.push([{ text: '🧪 وضع تجربة البوت (Sandbox)', callback_data: 'menu_sandbox' }]);
  ...
  // ✏️ Rename Student (Admin: /rename <telegram_id> <الاسم الجديد> | Student: /setname <الاسم الجديد>)
  bot.command(['rename', 'setname'], async (ctx) => {
    const fromId = ctx.from?.id;
    const text = ctx.message.text.trim();
    const parts = text.split(' ').filter(Boolean);

    if (parts[0].startsWith('/rename')) {
      if (Number(fromId) !== ADMIN_CHAT_ID) {
        return ctx.reply('⛔ هذا الأمر مخصص فقط لمدير المنظومة.');
      }
      if (parts.length < 3) {
        return ctx.reply('⚠️ الاستخدام: <code>/rename &lt;telegram_id&gt; &lt;الاسم الجديد&gt;</code>\nمثال: <code>/rename 862270793 د. علي أشرف</code>', { parse_mode: 'HTML' });
      }
    ...
    // Student self /setname
    if (parts.length < 2) {
      return ctx.reply('⚠️ الاستخدام: <code>/setname د. علي أشرف</code> أو <code>/setname أنا اسمي علي أشرف</code>', { parse_mode: 'HTML' });
    }
  ...
  bot.on('text', async (ctx) => {
    ...
    try {
      ...
      if (session?.state === 'waiting_custom_course') {
        ...
        const code = (parts[0] || 'MOD').trim();
        const title = (parts[1] || text).trim();

        const currentProf = await getUserProfile(fromId);
        const currentCustom = currentProf?.custom_courses || [];
        const updatedCustom = [...currentCustom, { code, title }];

        await updateUserAcademicProfile(fromId, { customCourses: updatedCustom });

        let successMsg = `✅ <b>تمت إضافة الموديول الجديد بنجاح! 🎓</b>\n`;
        successMsg += `━━━━━━━━━━━━━━━━━━━━━\n`;
        successMsg += `🩺 <b>الكود:</b> <code>${code}</code>\n`;
        successMsg += `📚 <b>الموديول:</b> <b>${title}</b>\n\n`;
  ...
  function getAcademicConfigContent(profile, courses) {
    ...
    const keyboard = {
      inline_keyboard: [
        ...
        [
          { text: '➕ إضافة موديول خاص بجامعتك', callback_data: 'menu_add_custom_course' }
        ],
        [
          { text: '🔙 القائمة الرئيسية', callback_data: 'menu_main' }
        ]
  ...
  for (const [btnAction, yearTitle] of Object.entries(yearMapping)) {
    bot.action(btnAction, async (ctx) => {
      const fromId = ctx.from?.id;
      const updatedProf = await updateUserAcademicProfile(fromId, { academicYear: yearTitle });
  ...
  for (const [btnAction, semTitle] of Object.entries(semMapping)) {
    bot.action(btnAction, async (ctx) => {
      const fromId = ctx.from?.id;
      const updatedProf = await updateUserAcademicProfile(fromId, { semester: semTitle });
