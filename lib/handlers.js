...
export function registerHandlers(bot) {
  ...
  bot.action(/^take_discipline_quiz(?:_(.+))?$/, async (ctx) => {
    ...
    try {
      ...
      await supabase.from('medical_spaced_quizzes').update({ repetition_level: 1 }).ilike('topic', `[UID:${fromId}]%`).eq('question', quiz.question);
    } catch (e) {}
  });
  ...
  bot.action(/^take_sahabi_quiz(?:_(.+))?$/, async (ctx) => {
    ...
    try {
      ...
      // Mark any pending spaced quiz as already answered
      await supabase.from('medical_spaced_quizzes').update({ repetition_level: 1 }).ilike('topic', `[UID:${fromId}]%`).eq('question', quiz.question);
    } catch (e) {}
  });
