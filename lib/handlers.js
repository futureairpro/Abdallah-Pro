  ...
  async function executeParsedLifeActions(ctx, result, fromId) {
    ...
    if (data.prayer_habits && Object.keys(data.prayer_habits).length > 0) {
      ...
      try {
        ...
        if (p.fajr && p.fajr !== 'لم يُسجل') prayerListText.push(`الفجر (${p.fajr})`);
        if (p.dhuhr && p.dhuhr !== 'لم يُسجل') prayerListText.push(`الظهر (${p.dhuhr})`);
        if (p.asr && p.asr !== 'لم يُسجل') prayerListText.push(`العصر (${p.asr})`);
        if (p.maghrib && p.maghrib !== 'لم يُسجل') prayerListText.push(`المغرب (${p.maghrib})`);
        if (p.isha && p.isha !== 'لم يُسجل') prayerListText.push(`العشاء (${p.isha})`);
