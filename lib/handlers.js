...
// 💤 Egyptian Dialect & Sleep Intelligence Parsers
export function parseTimeTo24h(timeStr) {
  if (!timeStr || typeof timeStr !== 'string') return null;
  const clean = timeStr.trim();
  const isPm = /م|مساء|عصر/i.test(clean);
  const isAm = /ص|صباح|فجر/i.test(clean);
  const m = clean.match(/(\d{1,2}):(\d{2})/);
  if (!m) {
    const singleDigit = clean.match(/(\d{1,2})/);
    if (!singleDigit) return null;
    let h = parseInt(singleDigit[1], 10);
    if (isPm && h < 12) h += 12;
    if (isAm && h === 12) h = 0;
    return { hours: h, minutes: 0, str24: `${String(h).padStart(2, '0')}:00` };
  }
  ...
  return {
    hours: h,
    minutes: min,
    str24: `${String(h).padStart(2, '0')}:${String(min).padStart(2, '0')}`
  };
}

export function calculateSleepDurationHours(bedtime24, wakeup24) {
  const b = parseTimeTo24h(bedtime24);
  const w = parseTimeTo24h(wakeup24);
  if (!b || !w) return 0;
  const bMinutes = b.hours * 60 + b.minutes;
  const wMinutes = w.hours * 60 + w.minutes;
  let diffMinutes = wMinutes - bMinutes;
  if (diffMinutes <= 0) diffMinutes += 24 * 60;
  const hours = diffMinutes / 60;
  return Math.round(hours * 10) / 10;
}

export function extractSleepWakeClauses(rawText) {
  const text = rawText || '';
  const sleepKeywords = ['نمت', 'نايم', 'النوم', 'نوم', 'دخلت أنام', 'روحت أنام', 'كنت نايم'];
  const wakeKeywords = ['صحيت', 'استيقظت', 'الاستيقاظ', 'صحيتني', 'قمت'];

  let sleepIdx = -1;
  for (const sk of sleepKeywords) {
    const idx = text.indexOf(sk);
    if (idx !== -1 && (sleepIdx === -1 || idx < sleepIdx)) sleepIdx = idx;
  }

  let wakeIdx = -1;
  for (const wk of wakeKeywords) {
    const idx = text.indexOf(wk);
    if (idx !== -1 && (wakeIdx === -1 || idx < wakeIdx)) wakeIdx = idx;
  }

  let sleepClause = '';
  let wakeClause = '';

  if (sleepIdx !== -1 && wakeIdx !== -1) {
    if (sleepIdx < wakeIdx) {
      sleepClause = text.slice(sleepIdx, wakeIdx);
      wakeClause = text.slice(wakeIdx);
    } else {
      wakeClause = text.slice(wakeIdx, sleepIdx);
      sleepClause = text.slice(sleepIdx);
    }
  } else if (sleepIdx !== -1) {
    sleepClause = text.slice(sleepIdx);
  } else if (wakeIdx !== -1) {
  ...
  return { sleepClause, wakeClause };
}

export function formatEgyptianTimeLabel(timeStr) {
  const t = parseTimeTo24h(timeStr);
  if (!t) return timeStr || '';
  const { hours, minutes } = t;
  const mStr = minutes > 0 ? `:${String(minutes).padStart(2, '0')}` : ':00';
  const h12 = (hours % 12) || 12;
  const m12Str = `${h12}${mStr}`;

  if (hours === 0 && minutes === 0) return '12:00 منتصف الليل';
  if (hours >= 0 && hours < 4) return `${m12Str} بالليل (صباحاً)`;
  if (hours >= 4 && hours < 6) return `${m12Str} الفجر`;
  if (hours >= 6 && hours < 12) return `${m12Str} صباحاً`;
  if (hours >= 12 && hours < 15) return `${m12Str} ظهراً`;
  if (hours >= 15 && hours < 18) return `${m12Str} عصراً`;
  if (hours >= 18 && hours < 20) return `${m12Str} المغرب`;
  return `${m12Str} بالليل`;
}

export function normalizeSleepData(p, userRawText = '') {
  if (!p || typeof p !== 'object') return p;
  const rawText = String(userRawText || '');
  const { sleepClause, wakeClause } = extractSleepWakeClauses(rawText);

  let bedtime = p.sleep_bedtime ? parseTimeTo24h(p.sleep_bedtime) : null;
  let wakeup = p.wake_up_time ? parseTimeTo24h(p.wake_up_time) : null;

  // Clause 1: Bedtime analysis
  if (bedtime) {
    const hasAsr = /(عصر|عصراً|العصر|بعد الظهر|مساء|مساءا|مساءً)/i.test(sleepClause);
    const hasFajr = /(فجر|الفجر)/i.test(sleepClause);
    const hasNight = /(بليل|بالليل|ليل|عشا)/i.test(sleepClause);

    if (hasAsr && bedtime.hours >= 1 && bedtime.hours <= 6) {
      bedtime.hours += 12;
    } else if (hasFajr && bedtime.hours >= 12) {
      bedtime.hours -= 12;
    } else if (hasNight) {
      if (bedtime.hours >= 7 && bedtime.hours <= 11) bedtime.hours += 12;
      else if (bedtime.hours === 12) bedtime.hours = 0;
    }
    bedtime.str24 = `${String(bedtime.hours).padStart(2, '0')}:${String(bedtime.minutes).padStart(2, '0')}`;
  }

  // Clause 2: Wakeup analysis
  if (wakeup) {
    const hasDhuhr = /(ظهر|الظهر|الضهر)/i.test(wakeClause);
    const hasNightOrEarly = /(بليل|بالليل|صباحا|صباحاً|الفجر|بعد منتصف الليل)/i.test(wakeClause);
    const hasMorning = /(صبح|الصبح)/i.test(wakeClause);

    if (hasDhuhr && wakeup.hours >= 1 && wakeup.hours <= 3) {
      wakeup.hours += 12;
    } else if (hasNightOrEarly && wakeup.hours >= 12 && wakeup.hours <= 15) {
      wakeup.hours -= 12;
    } else if (hasMorning && wakeup.hours >= 12) {
      wakeup.hours -= 12;
    }
    wakeup.str24 = `${String(wakeup.hours).padStart(2, '0')}:${String(wakeup.minutes).padStart(2, '0')}`;
  }

  // Biologically plausible duration check
  if (bedtime && wakeup) {
    let dur = calculateSleepDurationHours(bedtime.str24, wakeup.str24);

    if (dur > 15 && bedtime.hours < 12) {
      const altBedHours = bedtime.hours + 12;
      const altBedStr = `${String(altBedHours).padStart(2, '0')}:${String(bedtime.minutes).padStart(2, '0')}`;
      const altDur = calculateSleepDurationHours(altBedStr, wakeup.str24);
      if (altDur >= 2 && altDur <= 15) {
        bedtime.hours = altBedHours;
        bedtime.str24 = altBedStr;
        dur = altDur;
      }
    }

    if (dur > 15 && wakeup.hours >= 12) {
      const altWakeHours = wakeup.hours - 12;
      const altWakeStr = `${String(altWakeHours).padStart(2, '0')}:${String(wakeup.minutes).padStart(2, '0')}`;
      const altDur = calculateSleepDurationHours(bedtime.str24, altWakeStr);
      if (altDur >= 2 && altDur <= 15) {
        wakeup.hours = altWakeHours;
        wakeup.str24 = altWakeStr;
        dur = altDur;
      }
    }
  ...
  if (bedtime) p.sleep_bedtime = bedtime.str24;
  if (wakeup) p.wake_up_time = wakeup.str24;
...
// 🧠 High-Yield Contextual Memory Formatter for Abdullah's Journey OS (~50-80 tokens max)
export function formatConversationContextForPrompt(session, todayDate) {
  if (!session) return '';

  const parts = [];
  const recentLogs = Array.isArray(session.recent_logs) ? session.recent_logs : [];
  const todaySummary = session.today_summary || {};

  // 1. Today's key recorded milestones & activities (to prevent duplicate relapses / gym sessions)
  const todayItems = [];
  if (todaySummary.date === todayDate) {
    if (todaySummary.purity_relapse?.occurred) {
      todayItems.push(`• انتكاسة سوسو وبوبو: مسجلة بالفعل اليوم (${todaySummary.purity_relapse.time || 'صباحاً'}). ⚠️ تنبيه حاسم: إذا كان الطالب يفضفض أو يعبر عن خنقة أو ضيق أو يذكر ما فعله سابقاً اليوم ➔ لا تسجل انتكاسة جديدة (اجعل purity_log: null إجبارياً)، وسجل تفريغ مشاعره في data.mental_wellness وقدم له دعماً نفسياً واستشفاءً دافئاً. لا تسجل انتكاسة جديدة إلا إذا قال صراحة "انتكست تاني دلوقتي" أو "عملت كذا مرة تانية حالاً".`);
    }
    if (todaySummary.last_gym) {
      todayItems.push(`• تمرين جيم اليوم: مسجل (${todaySummary.last_gym.duration_minutes || 60} دقيقة - ${todaySummary.last_gym.workout_type || 'حديد'}). ⚠️ تنبيه: إذا كان الطالب يحدد نوع التمرينة أو العضلات (مثل "كانت تمرينة رجل" أو "بنج وباي") ➔ هذا ليس تمريناً جديداً، بل تحديث للتمرين المسجل! اضبط is_update_to_recent: true مع update_target: "fitness_gym" واحتفظ بمدة التمرين الأصلية (${todaySummary.last_gym.duration_minutes || 60} دقيقة).`);
    }
    if (todaySummary.last_study) {
      todayItems.push(`• جلسة مذاكرة اليوم: مسجلة (${todaySummary.last_study.duration_minutes || 0} دقيقة - ${todaySummary.last_study.course_code || ''} ${todaySummary.last_study.topic || ''}). ⚠️ إذا كان يكمل تفاصيلها أو صفحاتها ➔ اضبط is_update_to_recent: true مع update_target: "study".`);
    }
    if (todaySummary.last_finance) {
      todayItems.push(`• عملية مالية مسجلة مؤخراً: (${todaySummary.last_finance.amount} ج.م - ${todaySummary.last_finance.description || ''} - ${todaySummary.last_finance.payment_method || ''}). ⚠️ إذا كان يوضح وسيلة الدفع أو تفاصيل البند ➔ اضبط is_update_to_recent: true مع update_target: "finance".`);
    }
  }

  if (todayItems.length > 0) {
    parts.push(`📌 أحداث ومحطات تم تثبيتها في المنظومة اليوم (${todayDate}):\n${todayItems.join('\n')}`);
  }

  // 2. Last 2-3 interactions (within last 2 hours)
  const now = Date.now();
  const recentInteractions = recentLogs.filter(item => item && item.timestamp && (now - item.timestamp) < 2 * 60 * 60 * 1000).slice(-3);
  if (recentInteractions.length > 0) {
    const lines = recentInteractions.map(it => `- رسالة سابقة (${it.time || ''}): "${it.user_text}" ➔ المنظومة وثقت: [${it.system_action}]`);
    parts.push(`💬 آخر تفاعلات سابقة دارت مع الطالب مؤخراً:\n${lines.join('\n')}`);
  }

  if (parts.length === 0) return '';

  return `\n🧠 ════════════════════════════════════════════════════════════
🧠 سياق الذاكرة اللحظية وربط الرسائل (Contextual Awareness & Memory):
${parts.join('\n\n')}
قواعد التعامل مع السياق:
1. الربط الذكي والتحديث: إذا كانت رسالة الطالب الحالية مكملة أو مرتبطة برسالة سابقة (تحديد نوع تمرين، صفحات مذاكرة، توضيح لمصروف)، اضبط "is_update_to_recent": true، وضع "update_target" باسم القسم ("fitness_gym"|"study"|"finance"|"quran")، ولا تسجل نشاطاً جديداً مكرراً.
2. التمييز بين الفضفضة والحدث الجديد: إذا كان الطالب يتحدث عن أمر حدث مسبقاً اليوم (مثل انتكاسة سوسو وبوبو أو تشتت) بهدف الفضفضة والتعبير عن الضيق النفسي، سجل المشاعر في data.mental_wellness وقدم الدعم في conversational_reply، ولا تعد تسجيل الحدث كأنه وقع من جديد!
════════════════════════════════════════════════════════════\n`;
}
...
export function registerHandlers(bot) {
  ...
  bot.on(['voice', 'audio'], async (ctx) => {
    ...
    try {
      ...
      const userProf = ctx.userProfile || (await getUserProfile(fromId));
      const studentName = userProf?.full_name || (Number(fromId) === ADMIN_CHAT_ID ? 'د. عبدالله' : 'دكتور زميل');
      const activeCourses = (Array.isArray(userProf?.custom_courses) && userProf.custom_courses.length > 0)
        ? userProf.custom_courses
        : (PRESET_COURSES_BY_YEAR[userProf?.academic_year]?.[userProf?.semester] || PRESET_COURSES_BY_YEAR['الفرقة الرابعة']['الترم الأول']);
      ...
      const parsedResult = await parseWithGeminiPool(audioBuffer, aiKeys, true, studentName, {
        ...
        preferences: userProf?.preferences,
        gender: userProf?.gender,
        sessionMemory: formatConversationContextForPrompt(session, getCairoToday())
      });

      await ctx.deleteMessage(waitMsg.message_id).catch(() => {});

      return executeParsedLifeActions(ctx, parsedResult, fromId, session);
  ...
  bot.on('text', async (ctx) => {
    ...
    try {
      ...
      // 🧭 Check if user explicitly asks "اعمل ايه دلوقتي" / "انا تايه" / "خطة اليوم"
      const isEnglishTranscript = /(?:Output transcript|transcript|\bEN\b|\bAR\b|How can we learn|learn English)/i.test(text) || (text.match(/[a-zA-Z]{3,}/g) || []).length >= 5;
      const whatNowExactRegex = /^(?:قولي\s+)?(?:أعمل|اعمل)\s+(?:إيه|ايه)(?:\s+دلوقتي|\s+النهاردة)?$|^(?:أنا\s+|انا\s+)?تايه(?:\s+مش\s+عارف\s+(?:أعمل|اعمل)\s+إيه)?$|^خطة\s+(?:النهاردة|اليوم)$|^وجهني$|^what\s+should\s+i\s+do$|^what\s+to\s+do$/i;
      if (!isEnglishTranscript && text.length < 90 && whatNowExactRegex.test(text.trim())) {
        await ctx.deleteMessage(waitMsg.message_id).catch(() => {});
        return handleWhatToDoNow(ctx);
      }
      ...
      const isEngFluencySession = /(?:Output transcript|transcript|\bEN\b|\bAR\b)/i.test(textToParse) || (textToParse.match(/[a-zA-Z]{3,}/g) || []).length >= 10;
      let parsedResult;
      if (isEngFluencySession) {
        parsedResult = await parseEnglishFluencyTranscript(textToParse, aiKeys, studentName);
      } else {
        parsedResult = await parseWithGeminiPool(textToParse, aiKeys, false, studentName, {
          academicYear: userProf?.academic_year,
          semester: userProf?.semester,
          courses: activeCourses,
          preferences: userProf?.preferences,
  ...
  bot.on(['photo', 'document'], async (ctx) => {
    ...
    try {
      ...
      // 🥗 4. Auto-detected Nutrition Meal Photo
      if (analysis.detected_type === 'nutrition_meal' || (analysis.nutrition_meal && (analysis.nutrition_meal.calories || analysis.nutrition_meal.meal_name))) {
        const nut = analysis.nutrition_meal;
        const savedMeal = await logNutritionMeal(fromId, {
          meal_name: nut.meal_name || 'وجبة مصورة',
          ...
          notes: nut.nutrition_pearl || 'مسجلة برؤية الذكاء الاصطناعي عبر الكاميرا 📷',
          date: todayDate
        });
        ...
        let msg = `🥗 <b>تم تحليل وتوثيق الوجبة بالذكاء الاصطناعي البصري! 📸✨</b>\n━━━━━━━━━━━━━━━━━━━━━\n`;
        msg += `🍽️ <b>الوجبة:</b> <b>${nut.meal_name || 'وجبة متوازنة'}</b> (${nut.meal_type || 'وجبة'})\n\n`;
        msg += `⚡ <b>السعرات المقدرة:</b> ~<b>${nut.calories || 0} kcal</b>\n`;
        msg += `🥩 <b>البروتين:</b> ~${nut.protein_g || 0}g | 🍞 <b>الكارب:</b> ~${nut.carbs_g || 0}g | 🥑 <b>الدهون:</b> ~${nut.fats_g || 0}g\n`;
        if (nut.nutrition_pearl) msg += `\n💡 <b>نصيحة الطبيب الغذائية:</b> <i>${nut.nutrition_pearl}</i>\n`;
        msg += `━━━━━━━━━━━━━━━━━━━━━\n✅ <i>تم تحديث عداد السعرات والماكروز في لوحة تحكم الويب فوراً!</i>`;

        return ctx.reply(msg, {
          parse_mode: 'HTML',
          reply_markup: {
            inline_keyboard: [
              [{ text: '🥗 فتح حلقات التغذية في الويب آب', callback_data: 'menu_nutrition' }]
            ]
          }
        });
      }

      // 📋 5. General Medical Document / Slide Note
  ...
  async function executeParsedLifeActions(ctx, result, fromId, passedSession = null) {
    ...
    // 💬 6. Casual Conversation / Chat Intent (only if no actionable flashcards or habits were extracted)
    const hasEnglishCards = Array.isArray(result.data?.english_flashcards) && result.data.english_flashcards.length > 0;
    if (result.detected_type === 'conversational_chat' && !hasEnglishCards) {
    ...
    const currentSession = passedSession || (fromId ? await getUserSession(fromId) : null) || {};
    if (!currentSession.today_summary || currentSession.today_summary.date !== todayDate) {
      currentSession.today_summary = { date: todayDate };
    }

    let purityRelapseLogged = false;
    let recordedPurityTypes = [];
    let insertedGymData = null;
    let insertedStudyData = null;
    let insertedFinanceData = null;
    ...
    if (Number(fromId) === ADMIN_CHAT_ID && data.purity_log) {
      ...
      if (p.is_relapse) {
        const rawUserMsg = String(ctx.message?.text || ctx.message?.caption || result.raw_transcription || '').toLowerCase();
        const todayRelapse = currentSession.today_summary?.purity_relapse;
        const isExplicitSecondRelapse = /(?:تاني|تانية|تانيه|مرة تانية|مره تانيه|دلوقتي تاني|وقعت تاني|انتكست تاني)/.test(rawUserMsg);
        const alreadyRelapsedToday = Boolean(todayRelapse && todayRelapse.occurred && currentSession.today_summary?.date === todayDate);

        if (alreadyRelapsedToday && !isExplicitSecondRelapse) {
          // 🛡️ Venting & emotional reflection on an already-recorded relapse: DO NOT reset streak again!
          insertedSummary.push(
            `🛡️ <b>بروتوكول قلعة السيادة والتعافي (متابعة ${typeLabel}):</b>\n` +
            `   ├ 📌 <b>الحالة:</b> انتكاسة مسجلة بالفعل من وقت سابق اليوم (${todayRelapse.time || 'صباحاً'}).\n` +
            `   ├ 🧠 <b>التوجيه النفسي:</b> مشاعر الخنقة والندم دليل على صحوة ضميرك، وتراجع الدوبامين طبيعي بعد التعثر.\n` +
            `   └ 🚀 <b>خطوتك الآن:</b> تم تفريغ شحنة المشاعر في سجل الفضفضة.. لا تعزل نفسك، توضأ وتحرك فوراً!`
          );
          ...
          // Ensure mental wellness gets logged if not already
          if (!data.mental_wellness) {
            data.mental_wellness = {
              venting_content: rawUserMsg || `فضفضة عن انتكاسة ${typeLabel} السابقة`,
              stress_level: 5,
              mood_rating: 1,
              focus_clarity: 2,
              ai_therapeutic_feedback: 'التعثر لا يعني نهاية الحرب. مشاعرك مفهومة، ركز في استعادة اتزانك الآن ولا تجلد ذاتك.'
            };
          }
        } else {
          ...
          purityRelapseLogged = true;
          recordedPurityTypes = type === 'both' ? ['soso', 'bobo'] : [type];
    ...
    // 1. English Flashcards (Spaced Repetition & Fluency Engine)
    if (Array.isArray(data.english_flashcards) && data.english_flashcards.length > 0) {
      ...
      for (const card of data.english_flashcards) {
        try {
          const term = card.term_or_sentence || card.term || card.sentence;
          const translation = card.egyptian_translation || card.arabic_translation || card.definition || 'ترجمة مصرية دارجة';
          const example = card.example_sentence || card.example || null;
          const category = card.category || 'conversation';

          if (term) {
            // Stagger next review times: 1st card in 2 hours, 2nd in 3 hours, etc.
            const staggerHours = Math.min(24, 2 + Math.floor(cardIdx * 1.5));
            const nextReview = new Date(Date.now() + staggerHours * 3600 * 1000).toISOString();

            const { data: row } = await supabase.from('english_spaced_flashcards').insert({
              term_or_sentence: term,
              egyptian_translation: translation,
            ...
            if (row?.id) {
              ...
              savedCards.push({ term, translation });
            }
            cardIdx++;
          }
        } catch (e) {
      ...
      if (savedCards.length > 0) {
        let engSummary = `🗣️ <b>بنك الطلاقة الإنجليزية (Spaced Repetition):</b> تم استخراج وتوثيق <b>(${savedCards.length} كارت وجملة)</b> للتكرار المتباعد 🇬🇧✨\n`;
        const previewCards = savedCards.slice(0, 8);
        previewCards.forEach(c => {
          engSummary += `   • 🌟 <code>${c.term}</code> ➔ ${c.translation}\n`;
        });
        if (savedCards.length > 8) {
          engSummary += `   <i>...وتم حفظ (${savedCards.length - 8}) كروت أخرى ستصلك بمواعيدها للمراجعة!</i>\n`;
        }
        insertedSummary.push(engSummary.trim());
      }
    }
      ...
      for (const s of data.study) {
        try {
          ...
          if (rawTopic || courseCode || s.duration_minutes || s.hours) {
            ...
            const lastStudy = currentSession.today_summary?.last_study;
            const rawMsg = (ctx.message?.text || ctx.message?.caption || result.raw_transcription || '').toLowerCase();
            const isExplicitNewStudy = /(?:جلسة تانية|جلسه تانيه|ذاكرت تاني|بدأت مذاكرة تانية|مادة تانية)/.test(rawMsg);
            const isStudyUpdate = !isExplicitNewStudy && (
              (result.is_update_to_recent && result.update_target === 'study') ||
              (lastStudy?.id && (Date.now() - (lastStudy.timestamp || 0)) < 45 * 60 * 1000 && (!s.duration_minutes || s.duration_minutes === 0))
            );

            if (isStudyUpdate && lastStudy?.id) {
              const updatedCourse = (courseCode && courseCode !== 'CAD402') ? courseCode : (lastStudy.course_code || courseCode);
              const updatedTopic = (rawTopic && rawTopic !== 'جلسة مذاكرة') ? rawTopic : (lastStudy.topic || rawTopic);
              const updatedPages = s.pages_covered || lastStudy.pages_covered || null;
              const updatedRating = s.comprehension_rating || lastStudy.comprehension_rating || null;
              const updatedDuration = durMins > 0 ? durMins : Number(lastStudy.duration_minutes || 0);

              await supabase.from('study_sessions').update({
                course_code: updatedCourse,
                topic: `[usr:${fromId}] ${updatedTopic}`.trim(),
                pages_covered: updatedPages,
                comprehension_rating: updatedRating,
                duration_minutes: updatedDuration
              }).eq('id', lastStudy.id);

              const pageText = updatedPages ? ` | 📄 <b>${updatedPages} صفحة</b>` : '';
              const ratingText = updatedRating ? ` | 🧠 استيعاب: ${'⭐'.repeat(updatedRating)}` : '';
              insertedSummary.push(`🩺 <b>تحديث جلسة المذاكرة [${updatedCourse}]:</b> <b>${updatedTopic}</b>\n   └ ⏱️ <b>المدة المعتمدة:</b> ${updatedDuration} دقيقة${pageText}${ratingText}`);
    ...
    if (data.prayer_habits && Object.keys(data.prayer_habits).length > 0) {
      ...
      const rawUserMsg = ctx.message?.text || ctx.message?.caption || result.raw_transcription || '';
      normalizeSleepData(p, rawUserMsg);
      try {
        ...
        if (p.sleep_hours > 0 || p.wake_up_time || p.sleep_bedtime) {
          let sleepLine = '';
          if (p.sleep_bedtime && p.wake_up_time) {
            sleepLine = `نمت: <b>${formatEgyptianTimeLabel(p.sleep_bedtime)}</b> | صحيت: <b>${formatEgyptianTimeLabel(p.wake_up_time)}</b> ➔ المدة: <b>${p.sleep_hours} ساعات</b>`;
          } else if (p.sleep_bedtime) {
            sleepLine = `نمت: <b>${formatEgyptianTimeLabel(p.sleep_bedtime)}</b>${p.sleep_hours ? ` (المدة: ${p.sleep_hours} ساعات)` : ''}`;
          } else if (p.wake_up_time) {
            sleepLine = `صحيت: <b>${formatEgyptianTimeLabel(p.wake_up_time)}</b>${p.sleep_hours ? ` (المدة: ${p.sleep_hours} ساعات)` : ''}`;
          } else {
            sleepLine = `نمت (${p.sleep_hours} ساعات)`;
          }
          insertedSummary.push(`💤 <b>سجل النوم والاستيقاظ:</b> ${sleepLine}`);
        }
    ...
    if (Array.isArray(data.fitness_gym) && data.fitness_gym.length > 0) {
      for (const g of data.fitness_gym) {
        try {
          ...
          const rawMsg = (ctx.message?.text || ctx.message?.caption || result.raw_transcription || '').toLowerCase();
          const lastGym = currentSession.today_summary?.last_gym;
          
          const isExplicitNewGym = /(?:تمرين تاني|تمرينة تانية|تمرينه تانيه|رحت الجيم تاني|نزلت الجيم تاني|تمرين جديد|روحنا الجيم تاني)/.test(rawMsg);
          const isGymUpdate = !isExplicitNewGym && (
            (result.is_update_to_recent && (result.update_target === 'fitness_gym' || !result.update_target)) ||
            (lastGym?.id && (
              /(?:رجل|ظهر|كتف|باي|تراي|بنش|صدر|بطن|أرجل|ارجل|ساق|legs|push|pull|chest|back|arms|triceps|biceps|تمرينة|تمرين)/i.test(rawMsg) ||
              (Date.now() - (lastGym.timestamp || 0)) < 3 * 3600 * 1000
            ))
          );

          if (isGymUpdate && lastGym?.id) {
            const updatedWorkoutType = (g.workout_type && g.workout_type !== 'حديد وتمارين مقاومة') ? g.workout_type : (lastGym.workout_type || 'حديد وتمارين مقاومة');
            const updatedMuscles = rawMuscle || lastGym.muscle_groups || '';
            const hasExplicitDuration = /\b(\d+)\s*(?:دقيقة|ساعة|نص)\b/.test(rawMsg);
            const updatedDuration = hasExplicitDuration ? Number(g.duration_minutes || lastGym.duration_minutes) : Number(lastGym.duration_minutes || g.duration_minutes || 60);

            await supabase.from('fitness_gym_logs').update({
              workout_type: updatedWorkoutType,
              muscle_groups: `[usr:${fromId}] ${updatedMuscles}`.trim(),
              duration_minutes: updatedDuration,
              exercises_summary: g.exercises_summary || lastGym.exercises_summary || null,
              protein_grams: g.protein_grams ? Number(g.protein_grams) : undefined,
              water_liters: g.water_liters ? Number(g.water_liters) : undefined
            }).eq('id', lastGym.id);

            insertedSummary.push(`🏋️‍♂️ <b>تحديث تمرين الجيم:</b> <b>${updatedWorkoutType}</b>\n   └ ⏱️ <b>المدة المعتمدة:</b> ${updatedDuration} دقيقة${updatedMuscles ? ` | 💪 <b>العضلات:</b> ${updatedMuscles}` : ''}${g.protein_grams ? ` | 🥩 <b>البروتين:</b> ${g.protein_grams} جم` : ''}`);

            insertedGymData = {
              id: lastGym.id,
              workout_type: updatedWorkoutType,
              muscle_groups: updatedMuscles,
              duration_minutes: updatedDuration,
              timestamp: Date.now()
            };
          } else {
    ...
    // 🧠 Save Session Memory, Today's Highlights, and Master Undo State in User Session
    if (fromId) {
      const nowCairoTime12 = new Date().toLocaleTimeString('ar-EG', { timeZone: 'Africa/Cairo', hour: '2-digit', minute: '2-digit', hour12: true });

      if (insertedGymData) {
        currentSession.today_summary.last_gym = insertedGymData;
      }
      if (purityRelapseLogged) {
        currentSession.today_summary.purity_relapse = {
          occurred: true,
          types: recordedPurityTypes,
          time: nowCairoTime12,
          timestamp: Date.now()
        };
      }
      if (insertedStudyData) {
        currentSession.today_summary.last_study = insertedStudyData;
      }
      if (insertedFinanceData) {
        currentSession.today_summary.last_finance = insertedFinanceData;
      }

      // Add interaction to rolling recent_logs (keep last 4 items)
      const userTextSample = (ctx.message?.text || ctx.message?.caption || result.raw_transcription || 'تسجيل صوتي').slice(0, 120);
      const actionSummarySample = insertedSummary.length > 0
        ? insertedSummary.map(s => s.replace(/<[^>]+>/g, '').split('\n')[0]).slice(0, 2).join(' | ')
        : (result.summary_text || 'تفاعل مع المنظومة');

      if (!Array.isArray(currentSession.recent_logs)) {
        currentSession.recent_logs = [];
      }
      currentSession.recent_logs.push({
        time: nowCairoTime12,
        timestamp: Date.now(),
        user_text: userTextSample,
        system_action: actionSummarySample
      });
      if (currentSession.recent_logs.length > 4) {
        currentSession.recent_logs = currentSession.recent_logs.slice(-4);
      }

      // Save Undo state if undo items or habit snapshots exist
      if (recordedUndoItems.length > 0 || previousHabitsSnapshot || previousFastingSnapshot) {
    ...
    if (result.summary_text) {
      ...
      if (data.prayer_habits) {
        const ph = data.prayer_habits;
        if (ph.sleep_bedtime && parseTimeTo24h(ph.sleep_bedtime)?.hours >= 12) {
          cleanSummary = cleanSummary.replace(/(?:0?3:30|3 ونص|تلاتة ونص)\s*(?:فجراً|فجر|صباحاً|الصبح)/g, '3:30 عصراً');
        }
        if (ph.wake_up_time && parseTimeTo24h(ph.wake_up_time)?.hours < 12) {
          cleanSummary = cleanSummary.replace(/(?:0?1:00|1|واحدة)\s*(?:ظهراً|ظهر|الظهر)/g, '1:00 ليلاً (صباحاً)');
        }
      }
      card += `${cleanSummary}\n\n`;
    }
  ...
  // 🗣️ English Flashcards Bank & Fluency Review Command
  bot.command(['english', 'eng', 'انجليزي', 'إنجليزي', 'vocab'], async (ctx) => {
    const { data: cards } = await supabase.from('english_spaced_flashcards').select('*').order('created_at', { ascending: false }).limit(8);
    let msg = `🗣️ <b>بنك الكلمات والطلاقة الإنجليزية (Spaced Repetition) 🇬🇧✨</b>\n━━━━━━━━━━━━━━━━━━━━━\n`;
    if (!cards || cards.length === 0) {
      msg += `✨ <i>لا توجد كروت محفوظة بعد. أرسل أي محادثة أو جملة وسيترجمها ويجدولها البوت للمراجعة!</i>\n`;
    } else {
      msg += `📌 <b>آخر الكروت المجدولة في بنكك:</b>\n`;
      cards.forEach(c => {
        msg += `• 🌟 <b>${c.term_or_sentence}</b> (${c.egyptian_translation}) [مستوى: ${c.repetition_level || 0}/6]\n`;
      });
    }
    return ctx.reply(msg, {
      parse_mode: 'HTML',
      reply_markup: {
        inline_keyboard: [
          [{ text: '📚 استعراض كروتي بالتفصيل', callback_data: 'menu_eng_spaced' }],
          [{ text: '🔙 القائمة الرئيسية', callback_data: 'menu_main' }]
        ]
      }
    });
  ...
  bot.action(/^take_sharia_quiz(?:_(.+))?$/, async (ctx) => {
    ...
    const capsuleId = ctx.match?.[1];
    const { SHARIA_CAPSULES, getRandomShariaCapsule, getShariaCapsuleById } = await import('./sharia_sciences.js');
    ...
    try {
      ...
      // Delete any scheduled pending level-0 quiz so it won't trigger later
      await supabase.from('medical_spaced_quizzes')
        .delete()
        .ilike('topic', `[UID:${fromId}]%`)
        .eq('question', capsule.quiz.question)
  ...
  bot.action(/^take_discipline_quiz(?:_(.+))?$/, async (ctx) => {
    const fromId = ctx.from?.id;
    if (Number(fromId) !== ADMIN_CHAT_ID) return ctx.answerCbQuery('غير مصرح لك').catch(() => {});
    await ctx.answerCbQuery('🎯 سؤال علم الأعصاب!').catch(() => {});
    const discId = ctx.match?.[1];
    const { SCIENTIFIC_DISCIPLINE_INSIGHTS, getRandomScientificDiscipline } = await import('./admin_curriculum.js');
    const disc = discId ? (SCIENTIFIC_DISCIPLINE_INSIGHTS.find(d => d.id === discId) || getRandomScientificDiscipline()) : getRandomScientificDiscipline();
    const quiz = disc.quiz || disc.item?.quiz;
    if (!quiz) return;
    try {
      const pollMsg = await bot.telegram.sendQuiz(fromId, quiz.question, quiz.options, {
      ...
      await saveNativeQuizPoll(fromId, {
        poll_id: pollMsg.poll.id,
        course_code: 'DISCIPLINE',
        topic: disc.title || disc.item?.title,
  ...
  bot.action(/^take_ayah_quiz(?:_(.+))?$/, async (ctx) => {
    const fromId = ctx.from?.id;
    if (Number(fromId) !== ADMIN_CHAT_ID) return ctx.answerCbQuery('غير مصرح لك').catch(() => {});
    await ctx.answerCbQuery('🎯 سؤال أسباب النزول!').catch(() => {});
    const ayahId = ctx.match?.[1];
    const { QURAN_AYAHS_WITH_ASBAB, getRandomAyahWithAsbab } = await import('./admin_curriculum.js');
    const ayah = ayahId ? (QURAN_AYAHS_WITH_ASBAB.find(a => a.id === ayahId) || getRandomAyahWithAsbab()) : getRandomAyahWithAsbab();
    const quiz = ayah.quiz || ayah.item?.quiz;
    if (!quiz) return;
    try {
      const pollMsg = await bot.telegram.sendQuiz(fromId, quiz.question, quiz.options, {
        correct_option_id: quiz.correct_option_index,
      ...
      await saveNativeQuizPoll(fromId, {
        poll_id: pollMsg.poll.id,
        course_code: 'QURAN_ASBAB',
        topic: ayah.surah || ayah.item?.surah,
        question: quiz.question,
        options: quiz.options,
        correct_option_id: quiz.correct_option_index,
        explanation: quiz.explanation,
  ...
  bot.action(/^take_bukhari_quiz(?:_(.+))?$/, async (ctx) => {
    const fromId = ctx.from?.id;
    if (Number(fromId) !== ADMIN_CHAT_ID) return ctx.answerCbQuery('غير مصرح لك').catch(() => {});
    await ctx.answerCbQuery('🎯 سؤال صحيح البخاري!').catch(() => {});
    const bukhariId = ctx.match?.[1];
    const { BUKHARI_HADITHS, getRandomBukhariHadith } = await import('./admin_curriculum.js');
    const bukhari = bukhariId ? (BUKHARI_HADITHS.find(b => b.id === bukhariId) || getRandomBukhariHadith()) : getRandomBukhariHadith();
    const quiz = bukhari.quiz || bukhari.item?.quiz;
    if (!quiz) return;
    try {
      const pollMsg = await bot.telegram.sendQuiz(fromId, quiz.question, quiz.options, {
        correct_option_id: quiz.correct_option_index,
      ...
      await saveNativeQuizPoll(fromId, {
        poll_id: pollMsg.poll.id,
        course_code: 'BUKHARI',
        topic: bukhari.hadith_title || bukhari.item?.hadith_title,
        question: quiz.question,
        options: quiz.options,
        correct_option_id: quiz.correct_option_index,
        explanation: quiz.explanation,
  ...
  bot.action(/^take_sahabi_quiz(?:_(.+))?$/, async (ctx) => {
    const fromId = ctx.from?.id;
    if (Number(fromId) !== ADMIN_CHAT_ID) return ctx.answerCbQuery('غير مصرح لك').catch(() => {});
    await ctx.answerCbQuery('🎯 سؤال صحابي اليوم!').catch(() => {});
    const sahabiId = ctx.match?.[1];
    const { SAHABA_SPOTLIGHTS, getRandomSahabiSpotlight } = await import('./admin_curriculum.js');
    const sahabi = sahabiId ? (SAHABA_SPOTLIGHTS.find(s => s.id === sahabiId) || getRandomSahabiSpotlight()) : getRandomSahabiSpotlight();
    const quiz = sahabi.quiz || sahabi.item?.quiz;
    if (!quiz) return;
    try {
      const pollMsg = await bot.telegram.sendQuiz(fromId, quiz.question, quiz.options, {
        correct_option_id: quiz.correct_option_index,
      ...
      await saveNativeQuizPoll(fromId, {
        poll_id: pollMsg.poll.id,
        course_code: 'SAHABA_SPOTLIGHT',
        topic: sahabi.name || sahabi.item?.name,
        question: quiz.question,
        options: quiz.options,
        correct_option_id: quiz.correct_option_index,
        explanation: quiz.explanation,
  ...
  // 🌿 Quran Healing Protocol Actions
