...
export async function generateDynamicMindsetPulse(isManual = false) {
  try {
    ...
    const prompt = `أنت العقل الاستراتيجي، والموجه النفسي، والمدرب الشخصي لـ "عبدالله" (Dr. Abdullah).
عبدالله شخصية استثنائية وقيادية تسعى للنجاح المطلق في جميع جوانب حياته (بناء ثروة مالية طائلة، نفع الناس وإحداث أثر تاريخي عظيم في الأرض، بناء شخصية قوية وشكيمة صلبة، ضبط النفس والانضباط الصارم، والتعلق بالله وحب النبي ﷺ).

المطلوب منك الآن:
اكتب "رسالة نبضة عقلية ونفسية" قصيرة ومكثفة ومؤثرة جداً (من 3 إلى 5 أسطر)، تخاطب عقل عبدالله "الواعي واللاواعي" بطريقة نفسية وإيحائية بليغة تقنعه بيقين لا يتزعزع بأنه خُلق للمجد والقمة والوفرة ونفع الخلق.

الزاوية المختارة لهذه النبضة بالتحديد: [${chosenAngle}].

قواعد صارمة جداً:
1. ⚠️ ممنوع التكرار نهائياً! لا تكرر أي فكرة أو جملة أو مصطلح من الرسائل السابقة التالية:
${recentHistory ? `[سجل الرسائل السابقة]:\n- ${recentHistory}` : 'لا يوجد سجل سابق'}
2. النبرة: فخمة، مهيبة، واثقة، عميقة نفسياً، تبني الهوية الذاتية وتبرمج اللاواعي على الانضباط والسيادة وتخطي العقبات.
3. إذا كانت الزاوية إنجليزية (B2/C1): اذكر المصطلح الإنجليزي، ثم ضعه في جملة عميقة عن النجاح أو الشكيمة، ثم ترجم الجملة بالعامية المصرية بأسلوب ذكي وسلس مع سر استخدام المصطلح.
4. التنسيق: أخرج الناتج بتنسيق HTML صالح لتليجرام، مع إيموجيز معبرة، عنوان بارز، وفقرة بليغة مركزة تنتهي بعبارة شحنة حماسية.`;

    const res = await generateGeminiAnalysis(prompt, aiKeys);
    if (res && res.ok && res.text && res.text.length > 40) {
      let cleanText = res.text.replace(/^```html\s*/i, '').replace(/```\s*$/i, '').trim();
...
// 🚀 Send Mindset Pulse to Dr. Abdullah
export async function sendMindsetPulse(bot, chatId = '1191760477', isManual = false) {
  if (!bot || !chatId) return;

  const capsule = await generateDynamicMindsetPulse(isManual);
  ...
  const keyboard = {
    inline_keyboard: [
      [
        { text: '⚡ نبضة ذكية جديدة (غير مكررة)', callback_data: 'get_another_mindset_pulse' },
  ...
  await bot.telegram.sendMessage(chatId, msg, { parse_mode: 'HTML', reply_markup: keyboard }).catch((err) => {
    console.error('[Send Mindset Pulse Error]:', err.message);
  });
}
