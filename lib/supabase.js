...
export async function getStoredAiKeys() {
  const envKeys = (process.env.GEMINI_API_KEYS || process.env.GEMINI_API_KEY || '').split(/[\n,;]+/).map(k => k.trim()).filter(k => k.length > 20);
  try {
    ...
    const dbKeys = (data?.data?.GEMINI_API_KEYS || '').split(/[\n,;]+/).map(k => k.trim()).filter(k => k.length > 20);
    const merged = Array.from(new Set([...envKeys, ...dbKeys]));
    return merged.length > 0 ? merged.join(',') : (process.env.GEMINI_API_KEYS || '');
  } catch (e) {
...
const USER_PROFILE_CACHE_TTL = 120000; // 2 minutes in-memory cache

export function invalidateUserProfileCache(userId) {
  if (userId) userProfileCache.delete(Number(userId));
}
...
export async function getUserProfile(userId) {
  ...
  const cached = userProfileCache.get(numId);
  ...
  try {
    ...
    if (userRow) {
      ...
      userProfileCache.set(numId, { profile: profResult, timestamp: Date.now() });
      return profResult;
    }
    ...
    if (storedSession && storedSession.profile && (storedSession.profile.full_name || storedSession.profile.trial_ends_at || storedSession.profile.created_at)) {
      ...
      userProfileCache.set(numId, { profile: fallbackProf, timestamp: Date.now() });
      return fallbackProf;
    }
...
export async function registerUserProfile(userId, { fullName, username, university = 'كلية الطب البشري' }) {
  ...
  try {
    ...
    // 1. Try insert/upsert into users table (excluding gender column)
    const { gender: _g, ...userDbPayload } = profile;
    try {
      await supabase.from('users').upsert(userDbPayload);
    } catch (_) {}
    ...
    userProfileCache.set(numId, { profile, timestamp: Date.now() });
...
let lastRegisteredUsersFetchTime = 0;

export async function getAllRegisteredUsers() {
  const now = Date.now();
  if (cachedRegisteredUsers && (now - lastRegisteredUsersFetchTime < 300000)) {
    return cachedRegisteredUsers;
  }

  const defaultAdmin = [{ telegram_id: ADMIN_CHAT_ID, full_name: 'د. عبدالله (المؤسس والمدير)', role: 'admin', is_active: true, subscription_status: 'lifetime', days_remaining: 3650 }];

  try {
    const { data: rows, error } = await supabase
      .from('bot_sessions')
      .select('chat_id, data')
      .not('chat_id', 'in', '(999999,888888,777777)');

    if (error) {
      console.warn('[getAllRegisteredUsers DB Error]:', error.message);
      return cachedRegisteredUsers || defaultAdmin;
    }
    ...
    (rows || []).forEach(r => {
      ...
      if (cid && !isAdminUser(cid) && cid > 1000) {
    ...
    cachedRegisteredUsers = Array.from(usersMap.values());
    lastRegisteredUsersFetchTime = now;
    return cachedRegisteredUsers;
  } catch (e) {
...
export async function saveNativeQuizPoll(telegramId, quizData) {
  const numId = Number(telegramId);
  const pollId = quizData.poll_id || quizData.telegram_poll_id || null;
  const metaObj = {
    poll_id: pollId,
    options: quizData.options || [],
    correct_index: Number(quizData.correct_option_index || quizData.correct_option_id || 0),
  ...
  // Check if an existing quiz row matches this question; if so, update its poll_id and meta rather than creating duplicate
  try {
    const { data: existing } = await supabase
      .from('medical_spaced_quizzes')
      .select('*')
      .ilike('topic', `[UID:${numId}]%`)
      .eq('question', quizData.question)
      .order('created_at', { ascending: false })
      .limit(1);
    ...
    if (existing && existing.length > 0) {
      ...
      await supabase.from('medical_spaced_quizzes').update({
        doctor_pearl: `<<<QUIZ_META_START>>>${JSON.stringify(metaObj)}<<<QUIZ_META_END>>> ${quizData.explanation || ex.doctor_pearl || ''}`.trim()
      }).eq('id', ex.id);
      return ex;
    }
  } catch (_) {}
  ...
  const payload = {
    ...
    repetition_level: 1,
    next_review_at: new Date(Date.now() + 48 * 3600 * 1000).toISOString()
  };
...
export async function processStudentPollAnswer(pollId, selectedOptionIndex, telegramId) {
  ...
  try {
    ...
    // Spaced repetition intervals: 48h (2d) -> 120h (5d) -> 288h (12d) -> 672h (28d) -> 1440h (60d) -> 2160h (90d)
    const intervals = { 1: 48, 2: 120, 3: 288, 4: 672, 5: 1440, 6: 2160 };
    const nextHours = intervals[nextLevel] || 48;
...
export async function scheduleAdminSpacedQuiz(telegramId, { course_code = 'SHARIA', topic, question, options, correct_option_index = 0, explanation = '', delayHours = 2.0 }) {
  const numId = Number(telegramId);
  const nextDate = new Date(Date.now() + delayHours * 3600 * 1000).toISOString();

  // 🛡️ Deduplication check: Avoid multiple instances of the exact same question
  try {
    const { data: existing } = await supabase
      .from('medical_spaced_quizzes')
      .select('id, repetition_level, is_mastered, next_review_at')
      .ilike('topic', `[UID:${numId}]%`)
    ...
    if (existing && existing.length > 0) {
      const ex = existing[0];
      // If already active or mastered, skip duplicate insertion
      if (ex.is_mastered || ex.repetition_level > 0) {
        return { success: true, data: ex, alreadyScheduled: true };
      }
      // If pending at level 0, refresh next_review_at
      await supabase.from('medical_spaced_quizzes').update({ next_review_at: nextDate }).eq('id', ex.id);
      return { success: true, data: ex, alreadyScheduled: true };
    }
  } catch (_) {}
