...
export async function getUserProfile(userId) {
  ...
  try {
    ...
    if (storedSession) {
      ...
      const now = Date.now();
      const trialEnd = p.trial_ends_at ? new Date(p.trial_ends_at).getTime() : (now + 3 * 24 * 3600 * 1000);
      ...
      return {
        ...
        is_trial: status === 'trial' && isActive,
        trial_ends_at: p.trial_ends_at || new Date(trialEnd).toISOString(),
...
export async function getAllRegisteredUsers() {
  try {
    ...
    (rows || []).forEach(r => {
      ...
      if (cid && cid !== 999999 && cid !== 888888 && cid !== 777777 && cid !== ADMIN_CHAT_ID && cid > 1000) {
        const prof = r.data?.profile || {};
        const trialEnd = prof.trial_ends_at ? new Date(prof.trial_ends_at).getTime() : (now + 3 * 24 * 3600 * 1000);
